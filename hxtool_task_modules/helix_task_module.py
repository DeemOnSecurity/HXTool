...
class helix_task_module(task_module):
    ...
    def _write_fh(
        ...
        for audit_object in self.yield_audit_results(
            ...
            gz.write(
                json.dumps(audit_object, sort_keys=False).encode("utf-8")
                + "\n".encode("utf-8")
            )
        gz.close()
        gz_fh.seek(0)
    ...
    def run(
        ...
        try:
            ...
            if not resp:
                raise Exception(
                    "Unable to get upload link from URL {}: {}".format(url, resp)
                )
            resp = resp.json()
            self.logger.debug(
                "Uploading id {} from {} to {}".format(
                    bulk_acquisition_id, bulk_download_path, resp["url"]
                )
            )
            ...
            with tempfile.TemporaryFile(mode="r+b") as gz_fh:
                ...
                resp = requests.post(
                    resp["url"],
                    data=resp["fields"],
                    files={"file": (str(time.time()), gz_fh)},
                )
                if not resp:
                    raise Exception("Unable to upload: {} {}".format(resp, resp.text))
                self.logger.info(
                    "Uploaded id {} in {}".format(
                        bulk_acquisition_id, (time.time() - start)
                    )
                )
