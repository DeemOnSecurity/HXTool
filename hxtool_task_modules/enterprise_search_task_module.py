    ...
    def run(
        ...
        if script:
            ...
            if hx_api_object and hx_api_object.restIsSessionValid():
                (ret, response_code, response_data) = hx_api_object.restSubmitSweep(
                    script,
                    hostset_id,
                ...
                if ret:
                    ...
                    self.parent_task.name = "Enterprise Search ID: {}".format(
                        response_data["data"]["_id"]
                    )
                    self.logger.info(
                        "Enterprise Search ID: {} successfully submitted.".format(
                            result["enterprise_search_id"]
                        )
                    )
                else:
                    self.logger.error(
                        "Enterprise Search submission failed. Response code: {}, response data: {}".format(
