    ...
    def run(
        ...
        if script:
            ...
            if hx_api_object:
                ...
                if (
                        not has_macros
                        and self.parent_task.last_run
                        and self.parent_task.stored_result.get("bulk_acquisition_id", 0) > 1
                ):
                    (
                        ret,
                        response_code,
                        response_data,
                    ) = hx_api_object.restRefreshBulkAcq(
                        self.parent_task.stored_result["bulk_acquisition_id"]
                    )
                else:
                    (ret, response_code, response_data) = hx_api_object.restNewBulkAcq(
                        script,
                ...
                if ret and "_id" in response_data["data"]:
                    ...
                    self.parent_task.name = "Bulk Acquisition ID: {}".format(
                        response_data["data"]["_id"]
                    )
                    self.logger.info(
                        "Bulk acquisition ID {} submitted successfully.".format(
                            response_data["data"]["_id"]
                        )
                    )
                    ...
                    self.logger.error(
                        "Bulk acquisition submission failed. Response code: {}, response data: {}".format(
