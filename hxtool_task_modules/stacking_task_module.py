...
from src.hxtool_data_models import *
        ...
        try:
            ...
            if bulk_download_path:
                stack_job = hxtool_global.hxtool_db.stackJobGet(
                    profile_id=self.parent_task.profile_id,
                ...
                with AuditPackage(bulk_download_path) as audit_pkg:
                    audit_data = audit_pkg.get_audit(
                        generator=stack_model["audit_module"], open_only=True
                    )
                    if audit_data:
                        records = get_audit_records(
                            audit_data,
                            stack_model["audit_module"],
                            stack_model["item_name"],
                            fields=stack_model["fields"],
                            post_process=stack_model["post_process"],
                            hostname=host_name,
                        )
                        if records:
                            hxtool_global.hxtool_db.stackJobAddResult(
                            ...
                            self.logger.debug(
                                "Stacking records added to the database for host {}".format(
                                    host_name
                                )
                            )
                            ret = True
                            ...
                            self.logger.warn(
                                "Stacking: No audit data for {}".format(host_name)
                            )
                    ...
                    try:
                        ...
                        self.logger.warn(
                            "Failed to remove {}".format(
                                os.path.realpath(bulk_download_path)
                            )
                        )
