    ...
    def run(self, bulk_download_eid=None, agent_id=None, host_name=None):
        ...
        try:
            ...
            if bulk_download_job and bulk_download_job["stopped"] == False:
                ...
                if hx_api_object:
                    (ret, response_code, response_data) = hx_api_object.restGetBulkHost(
                        bulk_download_job["bulk_acquisition_id"], agent_id
                    )
                    if (
                            ret
                            and isinstance(response_data, dict)
                            and (
                            response_data["data"]["state"] == "COMPLETE"
                            and response_data["data"]["result"]
                    )
                    ):
                        self.logger.debug(
                            "Processing bulk download for host: {0}".format(host_name)
                        )
                        download_directory = make_download_directory(
                            hx_api_object.hx_host,
                            bulk_download_job["bulk_acquisition_id"],
                        )
                        full_path = os.path.join(
                            download_directory,
                            get_download_filename(host_name, agent_id),
                        )
                        (
                            ret,
                            response_code,
                            response_data,
                        ) = hx_api_object.restDownloadFile(
                            response_data["data"]["result"]["url"], full_path
                        )
                        if ret:
                            hxtool_global.hxtool_db.bulkDownloadUpdateHost(
                                bulk_download_eid, agent_id, downloaded=True
                            )
                            self.logger.debug(
                                "Bulk download for host {} successfully downloaded to {}".format(
                                    host_name, full_path
                                )
                            )
                            result["bulk_acquisition_id"] = bulk_download_job[
                                "bulk_acquisition_id"
                            ]
                            ...
                            and (
                                    ...
                                    in {"FAILED", "CANCELLED", "ABORTED"}
                                    or (
                                            response_code == 404
                                            and response_data["details"][0]["code"] == 1005
                                    )
                            )
                    ) or not ret:
                        self.logger.debug(
                            "Error! Controller returned code: {}, data: {}".format(
                                response_code, response_data
                            )
                        )
                        self.parent_task.stop()
                        hxtool_global.hxtool_db.bulkDownloadDeleteHost(
                            bulk_download_eid, agent_id
                        )
                        ret = False
                    elif ret:
                        self.logger.debug(
                            "Deferring bulk download task for: {}".format(host_name)
                        )
