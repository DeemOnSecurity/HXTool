...
# hx_tool imports
from src.hxtool_formatting import *
from src.hxtool_config import *
from src.hxtool_session import *
from src.hxtool_apicache import *

# Import HXTool API Flask blueprint
from src.hxtool_api import ht_api
    ...
    if request.args.get("indicator"):
        ...
        if ret:
            ...
            (
                ret,
                response_code,
                condition_class_presence,
            ) = hx_api_object.restGetCondition(ioccategory, myiocuri, "presence")
            (
                ret,
                response_code,
                condition_class_execution,
            ) = hx_api_object.restGetCondition(ioccategory, myiocuri, "execution")
...
def bulkdownload(hx_api_object):
    if request.args.get("id"):
        ...
        if ret:
            ...
            return "HX controller responded with code {0}: {1}".format(
...
def download_multi_file_single(hx_api_object):
    if "mf_id" in request.args and "acq_id" in request.args:
        ...
        if multi_file:
            file_records = list(
                filter(
                    lambda f: int(f["acquisition_id"])
                              == int(request.args.get("acq_id")),
                    multi_file["files"],
                )
            )
            if file_records and file_records[0]:
                # TODO: should multi_file be hardcoded?
                path = combine_app_path(
                    download_directory_base(),
                    hx_api_object.hx_host,
                    "multi_file",
                    request.args.get("mf_id"),
                    "{}_{}.zip".format(
                        file_records[0]["hostname"], request.args.get("acq_id")
                    ),
                )
                ...
                logger.info(
                    format_activity_log(
                        msg="multi-file acquisition download",
                        id=request.args.get("acq_id"),
                        user=session["ht_user"],
                        controller=session["hx_ip"],
                    )
                )
                return send_file(
                    path, attachment_filename=os.path.basename(path), as_attachment=True
                )
        else:
            return "HX controller responded with code {0}: {1}".format(
                response_code, response_data
...
def settings(hx_api_object):
    if request.method == "POST":
        hxtool_global.hxtool_scheduler.add_task_api_session(
            ...
            request.form.get("bguser"),
            request.form.get("bgpass"),
        )
        logger.info(
            format_activity_log(
                msg="background processing credentials action",
                action="set",
                profile=session["ht_profileid"],
                user=session["ht_user"],
                controller=session["hx_ip"],
            )
        )
    ...
    bgcreds = formatProfCredsInfo(
        (
                hxtool_global.hxtool_db.backgroundProcessorCredentialGet(
                    session["ht_profileid"]
                )
                is not None
        )
...
def login():
    fail = None
    if request.method == "POST" and "ht_user" in request.form:
        ht_profile = hxtool_global.hxtool_db.profileGet(
            request.form["controllerProfileDropdown"]
        )
        if ht_profile:
            ...
            (ret, response_code, response_data) = hx_api_object.restLogin(
                request.form["ht_user"],
                request.form["ht_pass"],
            ...
            if ret:
                ...
                session["hx_int_version"] = int(
                    "".join(str(i) for i in hx_api_object.hx_version)
                )
                session["hx_ip"] = hx_api_object.hx_host
                session[
                    "ht_database_engine"
                ] = hxtool_global.hxtool_db.database_engine
                logger.info(
                    format_activity_log(
                        msg="user logged in",
                        user=session["ht_user"],
    ...
    return render_template(
        "ht_login.html",
        hx_default_port=HXAPI.HX_DEFAULT_PORT,
        fail=fail,
        version=hxtool_vars.__version__,
    )
...
def init_db():
    ...
    if hxtool_global.hxtool_config.get_child_item("db", "type") == "mongodb":
        from src.hxtool_mongodb import hxtool_mongodb
        ...
        return hxtool_mongodb(
            ...
            hxtool_global.hxtool_config.get_child_item(
                "db", "auth_mechanism", "SCRAM-SHA-256"
            ),
            hxtool_global.hxtool_config.get_child_item("db", "db_name", "hxtool"),
        )
    else:
        from src.hxtool_tinydb import hxtool_tinydb
        ...
        return hxtool_tinydb(
            ...
            apicache_refresh_interval=hxtool_global.hxtool_config.get_child_item(
                "apicache", "refresh_interval"
            ),
            write_cache_size=0,
        )
...
def app_init(debug=False):
    ...
    app.session_interface = hxtool_session_interface(
        app, expiration_delta=hxtool_global.hxtool_config["network"]["session_timeout"]
    )
        ...
        if os.path.isfile(combine_app_path(file)):
            if os.path.isfile(combine_app_path(hxtool_vars.data_path, file)):
                try:
                    ...
                    f = input
                r = f(
                    "{} already exists in {}, do you want to overwrite it? (Note that this might be a default file that you can safely overwrite) (Y/N)?".format(
                        file, hxtool_vars.data_path
                    )
                )
                if r.strip().lower() != "y":
                    continue
            ...
            os.rename(
                combine_app_path(file), combine_app_path(hxtool_vars.data_path, file)
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="HXTool version {}".format(hxtool_vars.__version__),
        usage="%(prog)s -h for more information.",
    )
    parser.add_argument(
        "-debug",
        dest="debug",
        ...
        required=False,
        default=False,
        help="Enable debug mode logging. Note: Very verbose.",
    )
    parser.add_argument(
        "--clear-sessions",
        dest="clear_sessions",
        ...
        required=False,
        default=False,
        help="Clear stale sessions from the database. Note that this should be done by a scheduler task.",
    )
    parser.add_argument(
        "--clear-saved-tasks",
        dest="clear_saved_tasks",
        action="store_true",
        required=False,
        default=False,
        help="Clear saved tasks from the database.",
    )
    parser.add_argument(
        "-v",
        "--version",
        action="version",
        version="HXTool version {}".format(hxtool_vars.__version__),
    )
    ...
    if wsgi_logger:
        ...
        request_log_handler = logging.handlers.RotatingFileHandler(
            combine_app_path(hxtool_vars.log_path, "access.log"),
            maxBytes=50000,
            backupCount=5,
        )
        request_log_formatter = logging.Formatter(
    ...
    logger.info(
        "Application is running. Please point your browser to http{0}://{1}:{2}. Press Ctrl+C/Ctrl+Break to exit.".format(
    ...
    if hxtool_global.hxtool_config["network"]["ssl"] == "enabled":
        ...
        context.load_cert_chain(
            combine_app_path(
                hxtool_vars.data_path, hxtool_global.hxtool_config["ssl"]["cert"]
            ),
            combine_app_path(
                hxtool_vars.data_path, hxtool_global.hxtool_config["ssl"]["key"]
            ),
        )
        app.run(
            host=hxtool_global.hxtool_config["network"]["listen_address"],
