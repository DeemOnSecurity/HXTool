...
class hxtool_api_cache:
    def __init__(self, hx_api_object, profile_id, intervals, objectTypes):
        ...
        for objectType in objectTypes:
            if objectType in intervals.keys():
                try:
                    setattr(
                        self,
                        objectType + "_fetcher_interval",
                        intervals[objectType]["fetcher_interval"],
                    )
                    setattr(
                        self,
                        objectType + "_objects_per_poll",
                        intervals[objectType]["objects_per_poll"],
                    )
                    setattr(
                        self,
                        objectType + "_refresh_interval",
                        intervals[objectType]["refresh_interval"],
                    )
                except:
                    self.logger.error(
                        "Missing interval settings for {}, check configuration to enable cache".format(
                            objectType
                        )
                    )
                    exit(2)
                ...
                my_fetcher_task.add_step(
                    self, "apicache_fetcher", kwargs={"objectType": objectType}
                )
        ...
        stats = {k: {
            "settings": v,
            "stats": {"records": 0, "timeline": deque([], maxlen=1000)},
    ...
    def apicache_processor(
            self, currOffset, objectType, records, myCache, refresh_interval
    ):
        ...
        for record in records:
            ...
            if record["_id"] in myCache.keys():
                t = datetime.datetime.now() - datetime.datetime.strptime(
                    myCache[record["_id"]], "%Y-%m-%d %H:%M:%S"
                )
                if t.seconds > refresh_interval:
                    hxtool_global.hxtool_db.cacheUpdate(
                        self.profile_id, objectType, record["_id"], record
                    )
                    s_update += 1
                    self.logger.debug(
                        "{}: {} record updated: {}".format(
                            self.profile_id, objectType, record["_id"]
                        )
                    )
                    ...
                    if objectType == "host":
                        (
                            ...
                            response_code,
                            response_data,
                        ) = self.hx_api_object.restGetHostSysinfo(record["_id"])
                        if ret:
                            hxtool_global.hxtool_db.cacheUpdate(
                                self.profile_id,
                                "sysinfo",
                            ...
                            self.logger.debug(
                                "{}: Host sysinfo record updated: {}".format(
                                    self.profile_id, record["_id"]
                                )
                            )
            else:
                ...
                if objectType == "host":
                    ...
                    if ret:
                        hxtool_global.hxtool_db.cacheAddById(
                            self.profile_id,
                            "sysinfo",
                            record["_id"],
                            response_data["data"],
                        ...
                        self.logger.debug(
                            "{}: New sysinfo record added: {}".format(
                                self.profile_id, record["_id"]
                            )
                        )
        ...
        myStats = {
            "timestamp": s_end.strftime("%Y-%m-%d %H:%M:%S"),
            "processed": s_total,
            "updates": s_update,
            "additions": s_add,
        ...
        if s_update is not 0 or s_add is not 0:
            self.logger.info(
                "{}: [{}] {} records updated, {} records added in {} seconds".format(
                    self.profile_id,
                    objectType,
                    s_update,
                    s_add,
                    (s_end - s_start).total_seconds(),
                )
            )
    ...
    def apicache_fetcher(self, objectType):
        ...
        myCache = {
            cacheEntry["contentId"]: cacheEntry["update_timestamp"]
        ...
        if objectType == "host":
            while True:
                (ret, response_code, response_data) = self.hx_api_object.restListHosts(
                    offset=myoffset,
                    sort_term="_id+ascending",
                    limit=self.host_objects_per_poll,
                )
                if ret:
                    # Leave loop if no new records are returned
                    if len(response_data["data"]["entries"]) == 0:
                        break
                    myoffset = self.apicache_processor(
                        myoffset,
                        objectType,
                        response_data["data"]["entries"],
                        myCache,
                        self.host_refresh_interval,
                    )

        elif objectType == "alert":
            while True:
                (ret, response_code, response_data) = self.hx_api_object.restGetAlerts(
                    offset=myoffset,
                    sort_term="_id+ascending",
                    limit=self.alert_objects_per_poll,
                )
                if ret:
                    if len(response_data["data"]["entries"]) == 0:
                        break
            ...
            while True:
                (
                    ...
                    response_code,
                    response_data,
                ) = self.hx_api_object.restListTriages(
                    offset=myoffset,
                    sort_term="_id+ascending",
                    limit=self.triage_objects_per_poll,
                )
                if ret:
                    if len(response_data["data"]["entries"]) == 0:
                        break
                    myoffset = self.apicache_processor(
                        ...
                        response_data["data"]["entries"],
                        myCache,
                        self.triage_refresh_interval,
                    )

        elif objectType == "file":
            while True:
                (ret, response_code, response_data) = self.hx_api_object.restListFileaq(
                    offset=myoffset,
                    sort_term="_id+ascending",
                    limit=self.file_objects_per_poll,
                )
                if ret:
                    if len(response_data["data"]["entries"]) == 0:
                        break
            ...
            while True:
                (
                    ...
                    response_code,
                    response_data,
                ) = self.hx_api_object.restListDataAcquisitions(
                    offset=myoffset,
                    sort_term="_id+ascending",
                    limit=self.live_objects_per_poll,
                )
                if ret:
                    if len(response_data["data"]["entries"]) == 0:
                        break
