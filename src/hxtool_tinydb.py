...
from src.hxtool_db import hxtool_db
...
from src import hxtool_vars, hxtool_logging
from src.hx_lib import HXAPI
from src.hxtool_util import secure_uuid4
...
class hxtool_tinydb(hxtool_db):
    def __init__(
            ...
            db_file,
            apicache=False,
            apicache_refresh_interval=None,
            write_cache_size=10,
    ):
        ...
        try:
            ...
            logger.error(
                "%s is not a TinyDB formatted database. Please move or rename this file before starting HXTool.",
                db_file,
            )
            exit(1)
    ...
    def check_schema(self):
        ...
        if not current_schema_version:
            logger.warning(
                "The current HXTool database has no schema version set, a DB schema upgrade may be required."
            )
            if self.upgrade_schema():
                logger.info("Database schema upgraded successfully.")
                self._db.table("schema_version").insert(
                    {"schema_version": hxtool_vars.hxtool_schema_version}
                )
        elif current_schema_version < hxtool_vars.hxtool_schema_version:
            logger.warning(
                "The current HXTool database has a schema version: {} that is older than the current version of: {}, a DB schema upgrade may be required.".format(
                    current_schema_version, hxtool_vars.hxtool_schema_version
                )
            )
            if self.upgrade_schema():
                logger.info("Database schema upgraded successfully.")
                self._db.table("schema_version").update(
                    {"schema_version": hxtool_vars.hxtool_schema_version}, doc_ids=[1]
                )
            ...
            with self._lock:
                ...
                for r in self._db.table("stacking").all():
                    if "bulk_download_id" in r and r["bulk_download_id"] > 0:
                        b = self._db.table("bulk_download").get(
                            (tinydb.Query()["profile_id"] == r["profile_id"])
                            & (
                                    tinydb.Query()["bulk_download_id"]
                                    == r["bulk_download_id"]
                            )
                        )
                        if b:
                            self._db.table("stacking").update(
                                {"bulk_download_eid": b.doc_id}, doc_ids=[r.doc_id]
                            )
                            self._db.table("stacking").update(
                                tinydb.operations.delete("bulk_download_id"),
                                doc_ids=[r.doc_id],
                            )
                ...
                for r in self._db.table("file_listing").all():
                    if "bulk_download_id" in r and r["bulk_download_id"] > 0:
                        ...
                        if b:
                            self._db.table("file_listing").update(
                                {"bulk_download_eid": b.doc_id}, doc_ids=[r.doc_id]
                            )
                            self._db.table("file_listing").update(
                                tinydb.operations.delete("bulk_download_id"),
                ...
                for r in self._db.table("bulk_download").all():
                    if "bulk_download_id" in r and r["bulk_download_id"] > 0:
                        self._db.table("bulk_download").update(
                            {"bulk_acquisition_id": r["bulk_download_id"]},
                            doc_ids=[r.doc_id],
                        )
                        self._db.table("bulk_download").update(
                            tinydb.operations.delete("bulk_download_id"),
                    ...
                    if "post_download_handler" in r:
                        self._db.table("bulk_download").update(
                            {"task_profile": r["post_download_handler"]},
                            doc_ids=[r.doc_id],
                        )
                        self._db.table("bulk_download").update(
                            tinydb.operations.delete("post_download_handler"),
                            doc_ids=[r.doc_id],
                        )
    ...
    def profileCreate(self, hx_name, hx_host, hx_port):
        ...
        with self._lock:
            ...
            try:
                r = self._db.table("profile").insert(
                    {
    ...
    def profileGet(self, profile_id):
        with self._lock:
            return self._db.table("profile").get(
                (tinydb.Query()["profile_id"] == profile_id)
            )

    def profileUpdate(self, profile_id, hx_name, hx_host, hx_port):
        with self._lock:
            return self._db.table("profile").update(
                {"hx_name": hx_name, "hx_host": hx_host, "hx_port": hx_port},
                (tinydb.Query()["profile_id"] == profile_id),
            )
    ...
    def profileDelete(self, profile_id):
        ...
        with self._lock:
            return self._db.table("profile").remove(
                (tinydb.Query()["profile_id"] == profile_id)
            )

    def backgroundProcessorCredentialCreate(
            self, profile_id, hx_api_username, iv, salt, hx_api_encrypted_password
    ):
        r = None
        with self._lock:
            try:
                r = self._db.table("background_processor_credential").insert(
                    {
                        "profile_id": profile_id,
                        "hx_api_username": hx_api_username,
                        "iv": iv,
                        "salt": salt,
                        "hx_api_encrypted_password": hx_api_encrypted_password,
                    }
                )
            except:
    ...
    def backgroundProcessorCredentialRemove(self, profile_id):
        with self._lock:
            return self._db.table("background_processor_credential").remove(
                (tinydb.Query()["profile_id"] == profile_id)
            )

    def backgroundProcessorCredentialGet(self, profile_id):
        with self._lock:
            return self._db.table("background_processor_credential").get(
                (tinydb.Query()["profile_id"] == profile_id)
            )
    ...
    def alertList(self, profile_id):
        with self._lock:
            return self._db.table("alert").search(
                (tinydb.Query()["profile_id"] == profile_id)
            )

    def alertGet(self, profile_id, hx_alert_id):
        with self._lock:
            return self._db.table("alert").get(
                (tinydb.Query()["profile_id"] == profile_id)
    ...
    def bulkDownloadCreate(
            self, profile_id, hostset_name=None, hostset_id=None, task_profile=None
    ):
        r = None
        with self._lock:
            try:
    ...
    def bulkDownloadGet(
            self, bulk_download_eid=None, profile_id=None, bulk_acquisition_id=None
    ):
        if bulk_download_eid:
            with self._lock:
                return self._db.table("bulk_download").get(
                    doc_id=int(bulk_download_eid)
                )
        elif profile_id and bulk_acquisition_id:
            with self._lock:
                return self._db.table("bulk_download").get(
                    (tinydb.Query()["profile_id"] == profile_id)
                    & (tinydb.Query()["bulk_acquisition_id"] == bulk_acquisition_id)
                )

    def bulkDownloadList(self, profile_id):
        with self._lock:
            return self._db.table("bulk_download").search(
                (tinydb.Query()["profile_id"] == profile_id)
            )
    ...
    def bulkDownloadUpdateHost(
            self, bulk_download_eid, host_id, downloaded=None, hostname=None
    ):
        d = {}
        ...
        with self._lock:
            return self._db.table("bulk_download").update(
                self._db_update_nested_dict("hosts", host_id, d),
                doc_ids=[int(bulk_download_eid)],
            )

    def bulkDownloadDeleteHost(self, bulk_download_eid, host_id):
        with self._lock:
            return self._db.table("bulk_download").update(
                self._db_delete_from_nested_dict("hosts", host_id),
                doc_ids=[int(bulk_download_eid)],
            )

    def bulkDownloadDelete(self, bulk_download_eid):
        with self._lock:
            return self._db.table("bulk_download").remove(
                doc_ids=[int(bulk_download_eid)]
            )
    ...
    def fileListingAddResult(self, profile_id, bulk_download_eid, result):
        with self._lock:
            return self._db.table("file_listing").update(
                self._db_append_to_list("files", result),
                (tinydb.Query()["profile_id"] == profile_id)
    ...
    def fileListingGetByBulkId(self, profile_id, bulk_download_eid):
        with self._lock:
            result = self._db.table("file_listing").search(
                (tinydb.Query()["profile_id"] == profile_id)
                & (tinydb.Query()["bulk_download_eid"] == int(bulk_download_eid))
            )
    ...
    def fileListingList(self, profile_id):
        with self._lock:
            return self._db.table("file_listing").search(
                tinydb.Query()["profile_id"] == profile_id
            )
        ...
        try:
            with self._lock:
                return self._db.table("multi_file").update(
                    self._db_append_to_list("files", job), eids=[int(multi_file_id)]
                )
        except:
            return None
    ...
    def multiFileList(self, profile_id):
        with self._lock:
            return self._db.table("multi_file").search(
                tinydb.Query()["profile_id"] == profile_id
            )
    ...
    def multiFileUpdateFile(self, profile_id, multi_file_id, acquisition_id):
        try:
            with self._lock:
                doc_ids = self._db.table("multi_file").update(
                    self._db_update_dict_in_list(
                        "files", "acquisition_id", acquisition_id, "downloaded", True
                    ),
    ...
    def stackJobGet(self, stack_job_eid=None, profile_id=None, bulk_download_eid=None):
        if stack_job_eid:
            ...
            with self._lock:
                return self._db.table("stacking").get(
                    (tinydb.Query()["profile_id"] == profile_id)
                    & (tinydb.Query()["bulk_download_eid"] == bulk_download_eid)
                )

    def stackJobList(self, profile_id):
        with self._lock:
            return self._db.table("stacking").search(
                (tinydb.Query()["profile_id"] == profile_id)
            )
    ...
    def stackJobAddHost(self, profile_id, bulk_download_eid, hostname, agent_id):
        with self._lock:
            return self._db.table("stacking").update(
                self._db_append_to_list(
                    "hosts",
                    {"hostname": hostname, "agent_id": agent_id, "processed": False},
                ),
                (tinydb.Query()["profile_id"] == profile_id)
                & (tinydb.Query()["bulk_download_eid"] == int(bulk_download_eid)),
            )
    ...
    def stackJobAddResult(self, profile_id, bulk_download_eid, hostname, result):
        with self._lock:
            e_id = self._db.table("stacking").update(
                self._db_append_to_list("results", result),
                (tinydb.Query()["profile_id"] == profile_id)
                & (tinydb.Query()["bulk_download_eid"] == int(bulk_download_eid)),
            )
            return self._db.table("stacking").update(
                self._db_update_dict_in_list(
                    "hosts", "hostname", hostname, "processed", True
                ),
                doc_ids=e_id,
            )
    ...
    def sessionGet(self, session_id):
        with self._lock:
            return self._db.table("session").get(
    ...
    def sessionDelete(self, session_id):
        with self._lock:
            return self._db.table("session").remove(
                (tinydb.Query()["session_id"] == session_id)
            )
    ...
    def taskGet(self, profile_id, task_id):
        with self._lock:
            return self._db.table("tasks").get(
                (tinydb.Query()["profile_id"] == profile_id)
    ...
    def taskDelete(self, profile_id, task_id):
        with self._lock:
            return self._db.table("tasks").remove(
                (tinydb.Query()["profile_id"] == profile_id)
                & (tinydb.Query()["task_id"] == task_id)
            )
    ...
    def auditCreate(
            self, profile_id, host_id, hostname, generator, start_time, end_time, results
    ):
        with self._lock:
            return self._db.table("audits").insert(
                {
                    "profile_id": profile_id,
    ...
    def auditList(self, profile_id):
        with self._lock:
            return self._db.table("audits").get(
                (tinydb.Query()["profile_id"] == profile_id)
            )
    ...
    def auditDelete(self, profile_id, audit_id):
        with self._lock:
            return self._db.table("audits").remove(
                (tinydb.Query()["profile_id"] == profile_id)
                & (tinydb.Query()["audit_id"])
            )
    ...
    def ruleUpdateState(self, rule_id, state):
        with self._lock:
            return self._db.table("rules").update(
    ...
    def ruleAddLog(self, rule_id, message):
        with self._lock:
            ...
            if "log" in r.keys():
                ...
                log.append(
                    {
                        "c_timestamp": datetime.datetime.now().strftime(
                            "%Y-%m-%d %H:%M:%S"
                        ),
                        "message": message,
                    }
                ...
                rn = self._db.table("rules").update(
                    {"log": log}, (tinydb.Query()["id"] == rule_id)
                )
                return True
            else:
                return False
    ...
    def ruleAdd(self, profile_id, name, category, platform, create_user, rule, method):
        with self._lock:
            return self._db.table("rules").insert(
                {
                    "profile_id": profile_id,
                    "id": str(secure_uuid4()),
                    "state": 0,
                    ...
                    "category": category,
                    "platform": platform,
                    "create_timestamp": datetime.datetime.now().strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                    "update_timestamp": datetime.datetime.now().strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                    "create_user": create_user,
    ...
    def cacheGet(self, profile_id, cacheType, contentId):
        with self._lock:
            if self.apicache:
                r = self._db.table("ObjectCache").get(
                    (tinydb.Query()["profile_id"] == profile_id)
                    & (tinydb.Query()["type"] == cacheType)
                    & (tinydb.Query()["contentId"] == contentId)
                )
                if not r:
                    # print("{} - Cache Miss (no record)".format(cacheType))
                    return False
                else:
                    t = datetime.datetime.now() - datetime.datetime.strptime(
                        r["update_timestamp"], "%Y-%m-%d %H:%M:%S"
                    )
                    if (
                            self.apicache_refresh_interval is not None
                            and t.seconds > self.apicache_refresh_interval
                    ):
    ...
    def cacheFlagRemove(self, profile_id, cacheType, offset):
        with self._lock:
            return self._db.table("ObjectCache").update(
                {
                    "removed_timestamp": datetime.datetime.now().strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                    "removed": True,
                },
    ...
    def cacheAdd(self, profile_id, cacheType, data):
        with self._lock:
            return self._db.table("ObjectCache").insert(
                {
                    "profile_id": profile_id,
                    "type": cacheType,
                    "contentId": data["_id"],
    ...
    def _db_update_nested_dict(
            self, dict_name, dict_key, dict_values, update_timestamp=True
    ):
    ...
    def _db_update_dict_in_list(
            self, list_name, query_key, query_value, k, v, update_timestamp=True
    ):
        def transform(element):
