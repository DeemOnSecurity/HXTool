...
from src.hxtool_util import combine_app_path
...
class hxtool_config:
    ...
    DEFAULT_CONFIG = {
        ...
        "scheduler": {"thread_count": None, "defer_interval": 30},
        "headers": {},
        "cookies": {},
    }
    ...
    def __init__(self, config_file):
        ...
        if path.isfile(config_file):
            with open(config_file, "r") as config_file_handle:
                ...
                if not {"log_handlers", "network", "ssl", "scheduler"} <= set(
                        self._config.keys()
                ):
                ...
                if ("proxies" in self._config["network"] and len(
                        list(
                            filter(
                                lambda x: x in ["http", "https"],
                                self._config["network"]["proxies"],
                            ))) == 0):
                    logger.warning(
                        "Ignoring invalid proxy configuration! Please see "
                        "http://docs.python-requests.org/en/master/user/advanced/ "
                    )
                    del self._config["network"]["proxies"]
        else:
            logger.warning(
                "Unable to open config file: %s, loading default config.", config_file
            )
            ...
            if handler_name == "rotating_file_handler":
                ...
                if "file" in handler_config:
                    h = logging.handlers.RotatingFileHandler(
                        combine_app_path(handler_config["file"])
                    )
                ...
                if "facility" in handler_config:
                    facility = logging.handlers.SysLogHandler.facility_names.get(
                        handler_config["facility"]
                    )

                socket_type = socket.SOCK_DGRAM
                if (
                        "protocol" in handler_config
                        and handler_config["protocol"].lower() == "tcp"
                ):
