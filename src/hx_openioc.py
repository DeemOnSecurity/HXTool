...
def generate_indicator(root):
    ...
    if platform:
        ...
        logger.info(
            "No platform data found in {}, assuming all platforms.".format(
                root.get("id")
            )
        )
...
def process_ioc(ioc):
    ...
    indicator = {indicator_id: {"presence": [], "execution": []}}
    ...
    try:
        ...
        logger.warning(
            "Unicode name identified in IOC "
            + root.get("id")
            + ". Skipping, please rename using ASCII."
        )
        return None
    ...
    if conditions:
        for cond in conditions:
            ...
            if sample_token.split("/")[0] in valid_tokens:
                ...
                logger.warning(
                    "{} contains unsupported token {}, skipping.".format(
                        indicator_id, cond[0]["token"]
                    )
                )
                return None
    else:
...
def openioc_to_hxioc(xml_content):
    ...
    try:
        ...
        logger.warning(
            "OpenIOC 1.1 indicator parsing failed with exception: {}".format(e)
        )
        return None
