...
class HXAPI:
    ...
    def build_request(
        ...
        request = requests.Request(
            method=method, url=full_url, params=params, data=data, auth=auth
        )
        ...
        if method not in ["GET", "DELETE"]:
            ...
            self.logger.debug(
                "HTTP method is not GET or DELETE, Content-Type header set to: %s",
                content_type,
            )
    ...
    def handle_response(
            ...
            multiline_json_limit=DEFAULT_LIMIT,
            stream=False,
    ):
        ...
        try:
            ...
            if (content_type is not None and "Accept" in request.headers
                    and request.headers["Accept"].lower() not in
                    content_type.lower()):
                self.logger.info(
                    "Request Accept header '%s' does not match response Content-Type '%s'",
                    request.headers["Accept"].lower(),
                    content_type.lower(),
                )
            ...
            if content_type is not None and "json" in content_type.lower():
                ...
                if response_data.startswith("{"):
                    self.logger.info(
                        "Possible JSON in response without corresponding Content-Type header."
                    )
    ...
    def get_token(self, update_last_use_timestamp=True):
        self.logger.debug(
            "get_token called, update_last_use_timestamp=%s", update_last_use_timestamp
        )

        if not self.fe_token:
            self.logger.debug("fe_token is empty.")
        elif update_last_use_timestamp:
            self.fe_token["last_use_timestamp"] = HXAPI.dt_to_str(
    ...
    def _set_version(self):
        ...
        if ret:
            ...
            if self.hx_version[0] == 2:
                ...
                self.api_version = 2 if self.hx_version[1] < 3 else 3
    ...
    def restGetUrl(
        ...
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restLogin(self, hx_api_username, hx_api_password, auto_renew_token=False):
        ...
        request = self.build_request(
            self.build_api_route("token", min_api_version=1),
            auth=(hx_api_username, hx_api_password),
        )
    ...
    def restLogout(self):
        ...
        request = self.build_request(
            self.build_api_route("token", min_api_version=1), method="DELETE"
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restIsSessionValid(self):
        ...
        if current_token:
            last_use_delta = (
                                     datetime.datetime.utcnow()
                                     - HXAPI.dt_from_str(current_token["grant_timestamp"])
                             ).seconds / 60
            grant_time_delta = (
                                       datetime.datetime.utcnow()
                                       - HXAPI.dt_from_str(current_token["grant_timestamp"])
            ...
            self.logger.debug(
                "Token last_use_timestamp is: {}, grant_timestamp is: {}, last_use_delta is: {}, grant_time_delta is: {}".format(
                    current_token["last_use_timestamp"],
                    current_token["grant_timestamp"],
                    last_use_delta,
                    grant_time_delta,
                )
            )
        ...
        if self.auto_renew_token and not is_valid:
            self.logger.debug(
                "Token has expired and auto_renew_token is set, renewing token."
            )
            if current_token:
            ...
            (is_valid, response_code, response_data) = self.restLogin(
                self.hx_user, self._hx_password, self.auto_renew_token
            )
    ...
    def restCreateCategory(self, category_name, category_options={}):
        ...
        request = self.build_request(
            self.build_api_route("indicator_categories/{0}".format(category_name)),
            method="PUT",
            data=json.dumps(category_options),
        )
    ...
    def restAddIndicator(
        ...
        request = self.build_request(
            self.build_api_route("indicators/{0}".format(ioc_category)),
            method="POST",
    ...
    def restDeleteIndicator(self, indicator_category, indicator_name):
        ...
        request = self.build_request(
            self.build_api_route(
                "indicators/{0}/{1}".format(indicator_category, indicator_name)
            ),
            method="DELETE",
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
    ...
    def restDeleteCategory(self, indicator_category):
        ...
        request = self.build_request(
            self.build_api_route("indicator_categories/{0}".format(indicator_category)),
            method="DELETE",
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
    ...
    def restAddCondition(self, ioc_category, ioc_guid, condition_class, condition_data):
        ...
        request = self.build_request(
            self.build_api_route(
                "indicators/{0}/{1}/conditions/{2}".format(
                    ioc_category, ioc_guid, condition_class
                )
            ),
            method="POST",
            data=condition_data,
    ...
    def restGetCondition(self, ioc_category, ioc_uri, condition_class, limit=10000):
        ...
        request = self.build_request(
            self.build_api_route(
                "indicators/{0}/{1}/conditions/{2}?limit={3}".format(
                    ioc_category, ioc_uri, condition_class, limit
                )
            )
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
    ...
    def restDeleteCondition(
            self, ioc_category, ioc_uri, condition_class, condition_uuid
    ):

        request = self.build_request(
            self.build_api_route(
                "indicators/{0}/{1}/conditions/{2}/{3}".format(
                    ioc_category, ioc_uri, condition_class, condition_uuid
                )
            ),
            method="DELETE",
        )
    ...
    def restGetIndicatorFromCondition(self, condition_id):
        ...
        request = self.build_request(
            self.build_api_route("conditions/{0}/indicators".format(condition_id))
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restGetConditionDetails(self, condition_id):
        ...
        request = self.build_request(
            self.build_api_route("conditions/{0}".format(condition_id))
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restListAllAcquisitions(self, limit=DEFAULT_LIMIT, offset=0, filter_term={}):
        ...
        request = self.build_request(
            self.build_api_route("acqs"), method="GET", params=params
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restAcquireTriage(self, agent_id, timestamp=False):
        ...
        request = self.build_request(
            self.build_api_route("hosts/{0}/triages".format(agent_id)),
            method="POST",
            data=data,
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
    ...
    def restAcquireFile(self, agent_id, path, filename, mode=True):
        ...
        data = json.dumps(
            {"req_path": path, "req_filename": filename, "req_use_api": mode}
        )

        request = self.build_request(
            self.build_api_route("hosts/{0}/files".format(agent_id)),
            method="POST",
            data=data,
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
    ...
    def restNewAcquisition(self, agent_id, scriptname, script, skip_base64=False):
        ...
        request = self.build_request(
            self.build_api_route("hosts/{0}/live".format(agent_id)),
            method="POST",
            data=data,
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
    ...
    def restFileAcquisitionById(self, acq_id):
        ...
        request = self.build_request(
            self.build_api_route("acqs/files/{0}".format(acq_id)),
            method="GET",
            data=data,
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
    ...
    def restDataAcquisitionByID(self, acq_id):
        ...
        request = self.build_request(
            self.build_api_route("acqs/live/{0}".format(acq_id)),
            method="GET",
            data=data,
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
    ...
    def restDataCollectionByID(self, acq_id):
        ...
        request = self.build_request(
            self.build_api_route("acqs/live/{0}.mans".format(acq_id)),
            method="GET",
            data=data,
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
    ...
    def restListBulkHosts(
            self, bulk_id, limit=DEFAULT_LIMIT, offset=0, sort_term={}, filter_term={}
    ):
    ...
    def restGetBulkHost(self, bulk_id, host_id):
        ...
        request = self.build_request(
            self.build_api_route("acqs/bulk/{0}/hosts/{1}".format(bulk_id, host_id))
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restGetBulkDetails(self, bulk_id):
        ...
        request = self.build_request(
            self.build_api_route("acqs/bulk/{0}".format(bulk_id))
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restDownloadFile(
            self, url, destination_file_path=None, accept="application/octet-stream"
    ):
    ...
    def restNewBulkAcq(
        ...
        request = self.build_request(
            self.build_api_route("acqs/bulk"), method="POST", data=json.dumps(data)
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restRefreshBulkAcq(self, bulk_acquisition_id, state="ALL"):
        ...
        request = self.build_request(
            self.build_api_route(
                "acqs/bulk/{0}/actions/refresh?state={1}".format(
                    bulk_acquisition_id, state
                )
            ),
            method="POST",
        )
    ...
    def restListFileAcquisitionsHost(self, host_id):
        ...
        request = self.build_request(
            self.build_api_route("hosts/{0}/files".format(host_id))
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restListTriageAcquisitionsHost(self, host_id):
        ...
        request = self.build_request(
            self.build_api_route("hosts/{0}/triages".format(host_id))
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restListDataAcquisitionsHost(self, host_id):
        ...
        request = self.build_request(
            self.build_api_route("hosts/{0}/live".format(host_id))
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restListSearches(
            self, limit=DEFAULT_LIMIT, offset=0, sort_term=None, filter_term={}
    ):
    ...
    def restSubmitSweep(
        ...
        data = {"indicator": indicator, "host_set": {"_id": int(host_set)}}
    ...
    def restCancelJob(self, path, id):
        ...
        request = self.build_request(
            self.build_api_route("{0}/{1}/actions/stop".format(path, id)), method="POST"
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restDeleteJob(self, path, id):
        ...
        request = self.build_request(
            self.build_api_route("{0}/{1}".format(path, id)), method="DELETE"
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restGetSearchHosts(self, search_id):
        ...
        request = self.build_request(
            self.build_api_route("searches/{0}/hosts?errors=true".format(search_id))
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restGetSearchResults(self, search_id, limit=DEFAULT_LIMIT):
        ...
        request = self.build_request(
            self.build_api_route(
                "searches/{0}/results?limit={1}".format(search_id, limit)
            )
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
    ...
    def restGetAlertID(self, alert_id):
        ...
        request = self.build_request(
            self.build_api_route("alerts/{0}".format(alert_id))
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restGetAlerts(
            ...
            offset=0,
            has_share_mode=None,
            sort_term="reported_at+desc",
            filter_term={},
            resolution_term=None,
    ):
    ...
    def restGetAlertsHost(self, agent_id, limit=DEFAULT_LIMIT):
        ...
        request = self.build_request(
            self.build_api_route("alerts/filter"), method="POST", data=data
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request, multiline_json=True, multiline_json_limit=limit, stream=True
        )

        if ret:
            from operator import itemgetter

            sorted_alert_list = sorted(
                response_data, key=itemgetter("reported_at"), reverse=True
            )
    ...
    # NOTE: this function does not return data in the usual way, the response is a list of alerts
    def restGetAlertsTime(
            self, start_date, end_date, limit=DEFAULT_LIMIT, filters=False
    ):
    ...
    def restDeleteHostByID(self, agent_id):
        ...
        request = self.build_request(
            self.build_api_route("hosts/{0}".format(agent_id)), method="DELETE"
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restGetHostSysinfo(self, host_id):
        ...
        request = self.build_request(
            self.build_api_route("hosts/{0}/sysinfo".format(host_id))
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restGetContainmentStatus(self, host_id):
        ...
        request = self.build_request(
            self.build_api_route("hosts/{0}/containment".format(host_id))
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restRequestContainment(self, host_id):
        ...
        request = self.build_request(
            self.build_api_route("hosts/{0}/containment".format(host_id)), method="POST"
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restModifyHostset(
            self, hostset_name, hostset_id, addlist=None, removelist=None
    ):

        data = {"name": hostset_name, "changes": [{}]}
        ...
        request = self.build_request(
            self.build_api_route("host_sets/static/{0}".format(hostset_id)),
            method="PUT",
            data=json.dumps(data),
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
    ...
    def restListCustomConfigChannels(
            self,
            limit=DEFAULT_LIMIT,
            offset=0,
            sort_term=None,
    ...
    def restNewConfigChannel(self, name, description, priority, host_sets, conf):
        ...
        myhostsets = [{"_id": int(hs)} for hs in host_sets]
        try:
            data = json.dumps(
                {
                    "name": name,
                    ...
                    "configuration": json.loads(conf),
                }
            )

            request = self.build_request(
                self.build_api_route("host_policies/channels"), method="POST", data=data
            )
            (
                ret,
                response_code,
            ...
            self.logger.error(
                "Failed to parse custom config channel JSON. Please verify your configuration."
            )
    ...
    def restGetConfigChannel(self, channel_id):
        ...
        request = self.build_request(
            self.build_api_route("host_policies/channels/{0}".format(channel_id))
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restGetConfigChannelConfiguration(self, channel_id):
        ...
        request = self.build_request(
            self.build_api_route("host_policies/channels/{0}.json".format(channel_id))
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restGetPolicy(self, policy_id):
        ...
        request = self.build_request(
            self.build_api_route("policies/{0}".format(policy_id))
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
    ...
    def restDeletePolicy(self, policy_id):
        ...
        request = self.build_request(
            self.build_api_route("policies/{0}".format(policy_id)), method="DELETE"
        )
        (ret, response_code, response_data, response_headers) = self.handle_response(
            request
        )
