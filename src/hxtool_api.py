...
from src.hxtool_vars import HXTOOL_API_VERSION
from src.hxtool_scheduler import *
from hxtool_task_modules import *
from src.hx_openioc import openioc_to_hxioc
...
def hxtool_api_hostsets_list(hx_api_object):
    ...
    if ret:
        ...
        return app.response_class(
            response=json.dumps(r), status=rcode, mimetype="application/json"
        )
...
def getHealth(hx_api_object):
    ...
    return app.response_class(
        response=json.dumps(myHealth), status=200, mimetype="application/json"
    )
...
#################
@ht_api.route(
    "/api/v{0}/auditmanager/getaudits".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_auditmanager_getaudits(hx_api_object):
    ...
    if ret:
        ...
        bulk_acqs = {entry["_id"]: entry for entry in response_data["data"]["entries"]}
        response = {}
        mydata = []

        for res in r:
            res_data = eval(res["_id"])
            row = {key: value for key, value in res_data.items()}
            row["count"] = res["count"]
            row["action"] = res_data["bulk_acquisition_id"]

            if res_data["bulk_acquisition_id"] in bulk_acqs:
                row["created"] = bulk_acqs[res_data["bulk_acquisition_id"]][
                    "create_time"
                ]
                row["state"] = bulk_acqs[res_data["bulk_acquisition_id"]]["state"]
                row["name"] = bulk_acqs[res_data["bulk_acquisition_id"]]["comment"]
                row["hostset"] = bulk_acqs[res_data["bulk_acquisition_id"]]["host_set"][
                    "name"
                ]
            else:
                row["created"] = ""
        ...
        return app.response_class(
            response=json.dumps("Unable to list bulk acquisitions"),
            status=404,
...
@ht_api.route(
    "/api/v{0}/auditmanager/remove".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_auditmanager_remove(hx_api_object):
    ...
    return app.response_class(
        response=json.dumps(rmessage), status=rcode, mimetype="application/json"
    )
...
def hxtool_api_auditviewer_query(hx_api_object):
    ...
    if mstr["type"] == "find":
        try:
            ...
            return app.response_class(
                response=json.dumps(r), status=404, mimetype="application/json"
            )
        ...
        for res in r:
            ...
            for key in res[res["generator_item_name"]].keys():
                row[res["generator_item_name"] + "/" + key] = res[
                    res["generator_item_name"]
                ][key]
            mydata.append(row)
...
################
@ht_api.route(
    "/api/v{0}/acquisition/remove".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_acquisition_remove(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restDeleteFile(
...
def hxtool_api_acquisition_get(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restGetUrl(
        request.args.get("url")
    )
    (r, rcode) = create_api_response(ret, response_code, response_data)
    return app.response_class(
...
@ht_api.route(
    "/api/v{0}/acquisition/download".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_acquisition_download(hx_api_object):
    if request.args.get("id"):
        if request.args.get("content") == "json":
            (ret, response_code, response_data) = hx_api_object.restDownloadFile(
                request.args.get("id"), accept="application/json"
            )
        else:
            (ret, response_code, response_data) = hx_api_object.restDownloadFile(
                request.args.get("id")
        ...
        if ret:
            ...
            flask_response.headers["Content-Type"] = response_data.headers[
                "Content-Type"
            ]
            flask_response.headers["Content-Disposition"] = response_data.headers[
                "Content-Disposition"
            ]
            app.logger.info(
                format_activity_log(
...
@ht_api.route(
    "/api/v{0}/acquisition/new".format(HXTOOL_API_VERSION), methods=["GET", "POST"]
)
@valid_session_required
def hxtool_api_acquisition_new(hx_api_object):
    if request.method == "POST":
        ...
        myscript = hxtool_global.hxtool_db.scriptGet(request.args.get("scriptid"))[
            "script"
        ]
        do_skip_base64 = True
    ...
    (ret, response_code, response_data) = hx_api_object.restNewAcquisition(
        myAgentID, myScriptName, myscript, skip_base64=do_skip_base64
    )
    (r, rcode) = create_api_response(ret, response_code, response_data)
    app.logger.info(
        format_activity_log(
...
def hxtool_api_acquisition_file(hx_api_object):
    ...
    (ret, response_code, response_data) = hx_api_object.restAcquireFile(
        request.args.get("id"), filePath, fileName, mode
    )

    (r, rcode) = create_api_response(ret, response_code, response_data)
    app.logger.info(
        format_activity_log(
            msg="file acquisition",
            action="new",
            host=request.args.get("id"),
            user=session["ht_user"],
            controller=session["hx_ip"],
...
@ht_api.route(
    "/api/v{0}/acquisition/triage".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_acquisition_triage(hx_api_object):
    if request.args.get("type") == "standard":
        (ret, response_code, response_data) = hx_api_object.restAcquireTriage(
            request.args.get("id")
        )
    elif request.args.get("type") in ["1", "2", "4", "8"]:
        mytime = datetime.datetime.now() - datetime.timedelta(
            hours=int(request.args.get("type"))
        )
        (ret, response_code, response_data) = hx_api_object.restAcquireTriage(
            request.args.get("id"), mytime.strftime("%Y-%m-%d %H:%M:%S")
        )
    elif request.args.get("type") == "timestamp":
        (ret, response_code, response_data) = hx_api_object.restAcquireTriage(
            request.args.get("id"), request.args.get("timestamp")
        )

    (r, rcode) = create_api_response(ret, response_code, response_data)
    app.logger.info(
        format_activity_log(
            msg="triage acquisition",
            action="new",
            host=request.args.get("id"),
            user=session["ht_user"],
            controller=session["hx_ip"],
...
# Stop
@ht_api.route(
    "/api/v{0}/enterprise_search/stop".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_enterprise_search_stop(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restCancelJob(
        "searches", request.args.get("id")
    )
    (r, rcode) = create_api_response(ret, response_code, response_data)
    app.logger.info(
        format_activity_log(
            msg="enterprise search",
            action="stop",
...
# Remove
@ht_api.route(
    "/api/v{0}/enterprise_search/remove".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_enterprise_search_remove(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restDeleteJob(
        "searches", request.args.get("id")
    )
    (r, rcode) = create_api_response(ret, response_code, response_data)
    app.logger.info(
...
@ht_api.route(
    "/api/v{0}/enterprise_search/new/db".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_enterprise_search_new_db(hx_api_object):
    if "sweephostset" not in request.args or request.args["sweephostset"] == "false":
        return app.response_class(
            response=json.dumps("Please select a host set."),
            status=400,
            mimetype="application/json",
        )
    ...
    enterprise_search_task = hxtool_scheduler_task(
        session["ht_profileid"], "Enterprise Search Task", start_time=start_time
    )
    ...
    enterprise_search_task.add_step(
...
@ht_api.route(
    "/api/v{0}/enterprise_search/new/file".format(HXTOOL_API_VERSION), methods=["POST"]
)
@valid_session_required
...
def hxtool_api_hosts_config(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restGetUrl(
        "/hx/api/v3/hosts/" + request.args.get("id") + "/configuration/actual.json"
    )
...
def hxtool_api_hosts_get(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restGetHostSummary(
        request.args.get("id")
    )
    (r, rcode) = create_api_response(response_data=response_data)
    return app.response_class(
...
def hxtool_api_hosts_sysinfo(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restGetHostSysinfo(
        request.args.get("id")
    )
    (r, rcode) = create_api_response(response_data=response_data)
    return app.response_class(
...
def hxtool_api_hosts_contain(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restRequestContainment(
        request.args.get("id")
    )
    (r, rcode) = create_api_response(response_data=response_data)
    app.logger.info(
...
def hxtool_api_hosts_uncontain(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restRemoveContainment(
        request.args.get("id")
    )
    (r, rcode) = create_api_response(response_data=response_data)
    app.logger.info(
...
@ht_api.route(
    "/api/v{0}/hosts/contain/approve".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_hosts_contain_approve(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restApproveContainment(
        request.args.get("id")
    )
    (r, rcode) = create_api_response(response_data=response_data)
    app.logger.info(
...
def hxtool_api_hosts_remove(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restDeleteHostByID(
        request.args.get("id")
    )
    (r, rcode) = create_api_response(response_data=response_data)
    app.logger.info(
...
def hxtool_api_openioc_upload(hx_api_object):
    ...
    rawioc = fc.read()
    hxtool_global.hxtool_db.oiocCreate(
        request.form["iocname"], HXAPI.b64(rawioc), session["ht_user"]
    )
...
def hxtool_api_openioc_download(hx_api_object):
    ...
    buffer = BytesIO()
    buffer.write(
        json.dumps(HXAPI.b64(myiocData["ioc"], decode=True, decode_string=True)).encode(
            default_encoding
        )
    )
    buffer.seek(0)
    ...
    app.logger.info(
        format_activity_log(
            msg="openioc action",
            action="download",
            name=myiocData["iocname"],
            user=session["ht_user"],
            controller=session["hx_ip"],
        )
    )
    return send_file(
        buffer, attachment_filename=myiocData["iocname"] + ".ioc", as_attachment=True
    )
...
def hxtool_api_alerts_get(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restGetAlertID(
        request.args.get("id")
    )
    # Workaround for matching condition which isn't a part of the response
    if (
            response_data.get("data", None) is not None
            and response_data["data"]["source"] == "IOC"
    ):
        ...
        if response_data["data"].get("indicator", None) is None:
            (
                ...
                cresponse_code,
                cresponse_data,
            ) = hx_api_object.restGetIndicatorFromCondition(
                response_data["data"]["condition"]["_id"]
            )
            if cret and len(cresponse_data["data"]["entries"]) > 0:
                tlist = [_["name"] for _ in cresponse_data["data"]["entries"]]
                response_data["data"].update(
                    {"indicator": {"display_name": "; ".join(tlist)}}
                )
        (cret, cresponse_code, cresponse_data) = hx_api_object.restGetConditionDetails(
            response_data["data"]["condition"]["_id"]
        )
        if ret:
            response_data["data"]["condition"]["tests"] = cresponse_data["data"][
                "tests"
            ]
            (r, rcode) = create_api_response(ret, response_code, response_data)
            return app.response_class(
...
def hxtool_api_annotation_add(hx_api_object):
    ...
    hxtool_global.hxtool_db.alertAddAnnotation(
...
@ht_api.route(
    "/api/v{0}/annotation/alert/view".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_annotation_alert_view(hx_api_object):
    alertAnnotations = hxtool_global.hxtool_db.alertGet(
        session["ht_profileid"], request.args.get("id")
    )
    return app.response_class(
        response=json.dumps(alertAnnotations), status=200, mimetype="application/json"
    )
...
def scheduler_health(hx_api_object):
    return app.response_class(
        response=json.dumps(hxtool_global.hxtool_scheduler.status()),
        status=200,
        mimetype="application/json",
    )
...
def scheduler_tasks(hx_api_object):
        ...
        if not task["parent_id"]:
            ...
            for subtask in hxtool_global.hxtool_scheduler.tasks():
                if subtask["parent_id"] == task["task_id"]:
                    if (
                            task_state_description.get(subtask["state"], "Unknown")
                            not in taskstates
                    ):
                        taskstates[
                            task_state_description.get(subtask["state"], "Unknown")
    ...
    return app.response_class(
        response=json.dumps(mytasks), status=200, mimetype="application/json"
    )
...
@ht_api.route(
    "/api/v{0}/taskprofile/remove".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
...
def hxtool_api_taskprofile_new(hx_api_object):
    ...
    hxtool_global.hxtool_db.taskProfileAdd(
        mydata["name"], session["ht_user"], mydata["params"]
    )
    (r, rcode) = create_api_response(ret=True)
    app.logger.info(
        format_activity_log(
...
# Remove
@ht_api.route(
    "/api/v{0}/acquisition/bulk/remove".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_acquisition_bulk_remove(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restDeleteJob(
        "acqs/bulk", request.args.get("id")
    )
    (r, rcode) = create_api_response(ret, response_code, response_data)
    app.logger.info(
        format_activity_log(
            msg="bulk acquisition action",
            action="remove",
            id=request.args.get("id"),
            user=session["ht_user"],
            controller=session["hx_ip"],
...
# Stop
@ht_api.route(
    "/api/v{0}/acquisition/bulk/stop".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
...
# Stop download
@ht_api.route(
    "/api/v{0}/acquisition/bulk/stopdownload".format(HXTOOL_API_VERSION),
    methods=["GET"],
)
@valid_session_required
def hxtool_api_acquisition_bulk_stopdownload(hx_api_object):
    ret = hxtool_global.hxtool_db.bulkDownloadUpdate(
        request.args.get("id"), stopped=True
    )
    app.logger.info(
        format_activity_log(
            msg="bulk acquisition action",
...
# Download
@ht_api.route(
    "/api/v{0}/acquisition/bulk/download".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_acquisition_bulk_download(hx_api_object):
    hostset_id = -1
    (ret, response_code, response_data) = hx_api_object.restGetBulkDetails(
        request.args.get("id")
    )
    if ret and "host_set" in response_data["data"]:
        hostset_id = int(response_data["data"]["host_set"]["_id"])

    (ret, response_code, response_data) = hx_api_object.restListBulkHosts(
        request.args.get("id")
    )

    if ret and response_data and len(response_data["data"]["entries"]) > 0:
        bulk_download_eid = hxtool_global.hxtool_db.bulkDownloadCreate(
            session["ht_profileid"], hostset_id=hostset_id, task_profile=None
        )

        bulk_acquisition_hosts = {}
        task_list = []
        for host in response_data["data"]["entries"]:
            bulk_acquisition_hosts[host["host"]["_id"]] = {
                "downloaded": False,
                "hostname": host["host"]["hostname"],
            }
            bulk_acquisition_download_task = hxtool_scheduler_task(
                session["ht_profileid"],
                "Bulk Acquisition Download: {}".format(host["host"]["hostname"]),
            )
            bulk_acquisition_download_task.add_step(
        ...
        hxtool_global.hxtool_db.bulkDownloadUpdate(
            bulk_download_eid,
            hosts=bulk_acquisition_hosts,
            bulk_acquisition_id=int(request.args.get("id")),
        )
        ...
        app.logger.warn(
            format_activity_log(
                msg="bulk acquisition action",
                action="download",
                error="No host entries were returned for bulk acquisition",
                id=request.args.get("id"),
                user=session["ht_user"],
                controller=session["hx_ip"],
            )
...
@ht_api.route(
    "/api/v{0}/acquisition/bulk/new/db".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_acquisition_bulk_new_db(hx_api_object):
    ...
    bulk_acquisition_script = hxtool_global.hxtool_db.scriptGet(
        request.args.get("bulkscript")
    )["script"]
    skip_base64 = True
...
@ht_api.route(
    "/api/v{0}/acquisition/bulk/new/file".format(HXTOOL_API_VERSION), methods=["POST"]
)
@valid_session_required
...
def hxtool_api_scripts_upload(hx_api_object):
    ...
    rawscript = fc.read()
    hxtool_global.hxtool_db.scriptCreate(
        request.form["scriptname"], HXAPI.b64(rawscript), session["ht_user"]
    )
    (r, rcode) = create_api_response(ret=True)
    app.logger.info(
        format_activity_log(
...
def hxtool_api_scripts_builder(hx_api_object):
    ...
    hxtool_global.hxtool_db.scriptCreate(
        mydata["scriptName"],
        HXAPI.b64(json.dumps(mydata["script"], indent=4).encode()),
        session["ht_user"],
    )
    app.logger.info(
        format_activity_log(
            msg="new scriptbuilder acquisiton script",
            name=mydata["scriptName"],
            user=session["ht_user"],
            controller=session["hx_ip"],
        )
    )
...
def hxtool_api_scripts_download(hx_api_object):
    ...
    buffer = BytesIO()
    buffer.write(
        HXAPI.b64(myscriptData["script"], decode=True, decode_string=True).encode(
            default_encoding
        )
    )
    buffer.seek(0)
    ...
    return send_file(
        buffer,
        attachment_filename=myscriptData["scriptname"] + ".json",
        as_attachment=True,
    )
...
########################
@ht_api.route(
    "/api/v{0}/indicator_category/get_edit_policies".format(HXTOOL_API_VERSION),
    methods=["GET"],
)
@valid_session_required
def hxtool_api_indicator_category_get_edit_policies(hx_api_object):
    ...
    if ret:
        mycategories = {
            category["_id"]: category["ui_edit_policy"]
...
@ht_api.route(
    "/api/v{0}/indicator_category/remove".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_indicator_category_remove(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restDeleteCategory(
        request.args.get("id")
    )
    (r, rcode) = create_api_response(ret, response_code, response_data)
    app.logger.info(
        format_activity_log(
            msg="rule category action",
            action="remove",
            id=request.args.get("id"),
            user=session["ht_user"],
            controller=session["hx_ip"],
...
@ht_api.route(
    "/api/v{0}/indicator_category/list".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
...
@ht_api.route(
    "/api/v{0}/indicator_category/new".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_indicator_category_new(hx_api_object):
    ...
    (ret, response_code, response_data) = hx_api_object.restCreateCategory(
        request.args.get("name"), category_options=mycategory_options
    )
    (r, rcode) = create_api_response(ret, response_code, response_data)
    app.logger.info(
        format_activity_log(
...
def hxtool_api_conditions_get(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restGetConditionDetails(
        request.args.get("id")
    )
    (r, rcode) = create_api_response(ret, response_code, response_data)
    return app.response_class(
...
###################
@ht_api.route(
    "/api/v{0}/indicatorqueue/remove".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_indicatorqueue_remove(hx_api_object):
    ...
    return app.response_class(
        response=json.dumps(r), status=200, mimetype="application/json"
    )


@ht_api.route(
    "/api/v{0}/indicatorqueue/view".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
...
@ht_api.route(
    "/api/v{0}/indicatorqueue/approve".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_indicatorqueue_approve(hx_api_object):
    r = hxtool_global.hxtool_db.ruleUpdateState(request.args.get("id"), 1)
    r = hxtool_global.hxtool_db.ruleAddLog(
        request.args.get("id"), "User " + session["ht_user"] + " approved this request"
    )
    return app.response_class(
        response=json.dumps("OK"), status=200, mimetype="application/json"
    )
...
@ht_api.route(
    "/api/v{0}/indicatorqueue/deny".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_indicatorqueue_deny(hx_api_object):
    r = hxtool_global.hxtool_db.ruleUpdateState(request.args.get("id"), 2)
    r = hxtool_global.hxtool_db.ruleAddLog(
        request.args.get("id"), "User " + session["ht_user"] + " denied this request"
    )
    return app.response_class(
        response=json.dumps("OK"), status=200, mimetype="application/json"
    )
...
@ht_api.route(
    "/api/v{0}/indicatorqueue/import".format(HXTOOL_API_VERSION), methods=["POST"]
)
@valid_session_required
def hxtool_api_indicatorqueue_import(hx_api_object):
    ...
    for iockey in iocs:
        hxtool_global.hxtool_db.ruleAdd(
            session["ht_profileid"],
            iocs[iockey]["name"],
            iocs[iockey]["category"],
            iocs[iockey]["platforms"],
            session["ht_user"],
            HXAPI.b64(json.dumps(iocs[iockey])),
            "add",
        )
...
def hxtool_api_indicators_remove(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restGetUrl(
        request.args.get("url"), method="DELETE"
    )
    (r, rcode) = create_api_response(ret, response_code, response_data)
    app.logger.info(
        format_activity_log(
...
@ht_api.route(
    "/api/v{0}/indicators/get/conditions".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_indicators_get_conditions(hx_api_object):
    url = request.args.get("url")
    (ret, response_code, condition_class_presence) = hx_api_object.restGetUrl(
        url + "/conditions/presence", include_params=True
    )
    (ret, response_code, condition_class_execution) = hx_api_object.restGetUrl(
        url + "/conditions/execution", include_params=True
    )

    myconditions = {
        "presence": condition_class_presence,
        "execution": condition_class_execution,
    }
...
@ht_api.route(
    "/api/v{0}/indicators/export".format(HXTOOL_API_VERSION), methods=["POST"]
)
@valid_session_required
def hxtool_api_indicators_export(hx_api_object):
    ...
    for uuid, ioc in iocList.items():
        (ret, response_code, response_data) = hx_api_object.restListIndicators(
            filter_term={"uri_name": ioc["uri_name"]}
        )
        if ret:
            iocList[uuid]["description"] = response_data["data"]["entries"][0][
                "description"
            ]
            iocList[uuid]["create_text"] = response_data["data"]["entries"][0][
                "create_text"
            ]
        (ret, response_code, response_data) = hx_api_object.restGetCondition(
            ioc["category"], ioc["uri_name"], "execution"
        )
        if ret:
            for item in response_data["data"]["entries"]:
                if "execution" not in iocList[uuid].keys():
                    iocList[uuid]["execution"] = []
                iocList[uuid]["execution"].append(item["tests"])
        (ret, response_code, response_data) = hx_api_object.restGetCondition(
            ioc["category"], ioc["uri_name"], "presence"
        )
        if ret:
            for item in response_data["data"]["entries"]:
                if "presence" not in iocList[uuid].keys():
    ...
    if len(iocList.keys()) == 1:
        ...
        buffer.write(
            json.dumps(iocList, indent=4, ensure_ascii=False).encode(default_encoding)
        )
    else:
        ...
        with zipfile.ZipFile(buffer, "w") as zf:
            for ioc in iocList:
                zf.writestr(
                    iocList[ioc]["uri_name"] + ".rule",
                    json.dumps(
                        {ioc: iocList[ioc]}, indent=4, ensure_ascii=False
...
@ht_api.route(
    "/api/v{0}/indicators/import".format(HXTOOL_API_VERSION), methods=["POST"]
)
@valid_session_required
    ...
    for file in files:
        ...
        try:
            ...
            if iocs is None:
                app.logger.warn(
                    format_activity_log(
                        msg="rule action fail",
                        reason="{} is not a valid Endpoint Security (HX) JSON or OpenIOC 1.1 indicator.".format(
                            file.filename
                        ),
                        action="import",
                        user=session["ht_user"],
        ...
        for iockey in iocs:
            ...
            (ret, response_code, response_data) = hx_api_object.restListCategories(
                limit=1, filter_term={"name": iocs[iockey]["category"]}
            )
            if ret:
                # As it turns out, filtering by name also returns partial matches. However the exact match seems to be the 1st result
                category_exists = (
                        len(response_data["data"]["entries"]) == 1
                        and response_data["data"]["entries"][0]["name"].lower()
                        == iocs[iockey]["category"].lower()
                )
                if not category_exists:
                    ...
                    (
                        ...
                        response_code,
                        response_data,
                    ) = hx_api_object.restCreateCategory(
                        HXAPI.compat_str(iocs[iockey]["category"])
                    )
                    category_exists = ret
                ...
                if category_exists:
                    (
                        ...
                        response_code,
                        response_data,
                    ) = hx_api_object.restAddIndicator(
                        iocs[iockey]["category"],
                        iocs[iockey]["name"],
                        create_text=iocs[iockey].get("create_text", None)
                                    or session["ht_user"],
                        platforms=iocs[iockey]["platforms"],
                        description=iocs[iockey].get("description", None),
                    )
                    if ret:
                        ...
                        if "presence" in iocs[iockey].keys():
                            for p_cond in iocs[iockey]["presence"]:
                                ...
                                (
                                    ...
                                    response_code,
                                    response_data,
                                ) = hx_api_object.restAddCondition(
                                    iocs[iockey]["category"], ioc_guid, "presence", data
                                )
                                if not ret:
                                    app.logger.warn(
                                        format_activity_log(
                                            msg="rule action fail",
                                            reason="failed to create presence condition",
                                            action="import",
                                            name=iocs[iockey]["name"],
                                            user=session["ht_user"],
                                            controller=session["hx_ip"],
                            ...
                            for e_cond in iocs[iockey]["execution"]:
                                ...
                                if not ret:
                                    app.logger.warn(
                                        format_activity_log(
                                            msg="rule action fail",
                                            reason="failed to create execution condition",
                                            action="import",
                                            name=iocs[iockey]["name"],
                                            user=session["ht_user"],
                                            controller=session["hx_ip"],
                    ...
                    app.logger.warn(
                        format_activity_log(
                            msg="rule action fail",
                            reason="unable to create category",
                            action="import",
                            name=iocs[iockey]["name"],
                            user=session["ht_user"],
                            controller=session["hx_ip"],
                ...
                app.logger.info(
                    format_activity_log(
                        msg="rule action",
                        reason="unable to import indicator",
                        action="import",
                        name=iocs[iockey]["name"],
                        user=session["ht_user"],
                        controller=session["hx_ip"],
...
def hxtool_api_indicators_new(hx_api_object):
    ...
    # REMOVE SOON
    (ret, response_code, response_data) = hx_api_object.restAddIndicator(
        mydata["category"],
        mydata["name"],
        session["ht_user"],
        chosenplatform,
    ...
    if ret:
        ...
        for key, value in mydata.items():
            if key not in ["name", "category", "platform", "description"]:
                ...
                (ret, response_code, response_data) = hx_api_object.restAddCondition(
                    mydata["category"], ioc_guid, ioctype, json.dumps(mytests)
                )
                if not ret:
                    ...
                    (
                        ...
                        response_code,
                        response_data,
                    ) = hx_api_object.restDeleteIndicator(mydata["category"], ioc_guid)
                    return (
                        "failed to create indicator conditions, check your conditions",
                        500,
                    )
        # All OK
        app.logger.info(
        ...
        app.logger.warn(
            format_activity_log(
                msg="rule action",
                action="new",
                reason="failed to create indicator",
                user=session["ht_user"],
                controller=session["hx_ip"],
            )
        )
    ...
    if ret:
        ...
        for key, value in mydata.items():
            if key not in [
                ...
                (ret, response_code, response_data) = hx_api_object.restAddCondition(
                    mydata["category"], myNewURI, ioctype, json.dumps(mytests)
                )
                if not ret:
        ...
        if myState:
            ...
            (ret, response_code, response_data) = hx_api_object.restDeleteIndicator(
                myOriginalCategory, myOriginalURI
            )
...
def hxtool_api_ccc_remove(hx_api_object):
    (ret, response_code, response_data) = hx_api_object.restDeleteConfigChannel(
        request.args.get("id")
    )
    (r, rcode) = create_api_response(ret, response_code, response_data)
    app.logger.info(
...
def hxtool_api_ccc_get(hx_api_object):
    (
        ...
        response_code,
        response_data,
    ) = hx_api_object.restGetConfigChannelConfiguration(request.args.get("id"))
    (r, rcode) = create_api_response(ret, response_code, response_data)
    return app.response_class(
        response=json.dumps(r), status=rcode, mimetype="application/json"
    )
...
############
@ht_api.route(
    "/api/v{0}/stacking/stacktypes".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_stacking_stacktypes(hx_api_object):
    mystacktypes = list(hxtool_data_models.stack_types.keys())
    return app.response_class(
        response=json.dumps(mystacktypes), status=200, mimetype="application/json"
    )
...
def hxtool_api_stacking_new(hx_api_object):
    ...
    if stack_type:
        ...
        bulk_download_eid = submit_bulk_job(
            hx_api_object,
            script_xml,
            hostset_id=hostset_id,
            task_profile="stacking",
            comment="HXTool Stacking Job: {}".format(stack_type["name"]),
        )
        ret = hxtool_global.hxtool_db.stackJobCreate(
            session["ht_profileid"], bulk_download_eid, request.form["stack_type"]
        )
        app.logger.info(
            format_activity_log(
                msg="stacking",
                action="new",
                hostsetid=hostset_id,
                type=request.form["stack_type"],
                user=session["ht_user"],
                controller=session["hx_ip"],
            )
        )
...
def hxtool_api_stacking_remove(hx_api_object):
    ...
    if stack_job:
        ret = True
        bulk_download_job = hxtool_global.hxtool_db.bulkDownloadGet(
            bulk_download_eid=stack_job["bulk_download_eid"]
        )
        if bulk_download_job and "bulk_acquisition_id" in bulk_download_job:
            (ret, response_code, response_data) = hx_api_object.restDeleteJob(
                "acqs/bulk", bulk_download_job["bulk_acquisition_id"]
            )
...
def hxtool_api_stacking_stop(hx_api_object):
    stack_job = hxtool_global.hxtool_db.stackJobGet(
        stack_job_eid=request.args.get("id")
    )
    if stack_job:
        bulk_download_job = hxtool_global.hxtool_db.bulkDownloadGet(
            bulk_download_eid=stack_job["bulk_download_eid"]
        ...
        if bulk_download_job and "bulk_acquisition_id" in bulk_download_job:
            ...
            hxtool_global.hxtool_db.bulkDownloadUpdate(
                bulk_download_job.doc_id, stopped=True
            )
            (r, rcode) = create_api_response(ret, response_code, response_data)
        else:
            (r, rcode) = create_api_response()
...
@ht_api.route(
    "/api/v{0}/acquisition/multi/file_listing/new".format(HXTOOL_API_VERSION),
    methods=["POST"],
)
@valid_session_required
def hxtool_api_acquisition_multi_file_listing(hx_api_object):
    ...
    try:
        ...
        if not display_name:
            display_name = "hostset: {0} path: {1} regex: {2}".format(
                hostset, path, regex
            )
    except re.error:
        return app.response_class(
            response=json.dumps("FAIL"), status=404, mimetype="application/json"
        )
    if script_xml:
        bulk_download_eid = submit_bulk_job(
            hx_api_object,
            HXAPI.compat_str(script_xml),
            hostset_id=hostset,
            task_profile="file_listing",
        )
        ret = hxtool_global.hxtool_db.fileListingCreate(
            session["ht_profileid"],
            session["ht_user"],
            bulk_download_eid,
            path,
        ...
        app.logger.info(
            format_activity_log(
                msg="multi-file listing acquisition",
                action="new",
                hostset_id=hostset,
                user=session["ht_user"],
                controller=session["hx_ip"],
...
@ht_api.route(
    "/api/v{0}/acquisition/multi/file_listing/stop".format(HXTOOL_API_VERSION),
    methods=["GET"],
)
@valid_session_required
def hxtool_api_acquisition_multi_file_listing_stop(hx_api_object):
    file_listing_job = hxtool_global.hxtool_db.fileListingGetById(
        request.args.get("id")
    )
    if file_listing_job:
        bulk_download_job = hxtool_global.hxtool_db.bulkDownloadGet(
            file_listing_job["bulk_download_eid"]
        )
        (ret, response_code, response_data) = hx_api_object.restCancelJob(
            "acqs/bulk", bulk_download_job["bulk_acquisition_id"]
        )
        if ret:
            ...
            hxtool_global.hxtool_db.bulkDownloadUpdate(
                file_listing_job["bulk_download_eid"], stopped=True
            )
            app.logger.info(
                format_activity_log(
                    msg="multi-file listing acquisition",
...
@ht_api.route(
    "/api/v{0}/acquisition/multi/file_listing/remove".format(HXTOOL_API_VERSION),
    methods=["GET"],
)
@valid_session_required
def hxtool_api_acquisition_multi_file_listing_remove(hx_api_object):
    ...
    if file_listing_job:
        ...
        hxtool_global.hxtool_db.bulkDownloadDelete(
            file_listing_job["bulk_download_eid"]
        )
...
@ht_api.route(
    "/api/v{0}/acquisition/multi/mf/stop".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
...
@ht_api.route(
    "/api/v{0}/acquisition/multi/mf/remove".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def hxtool_api_acquisition_multi_mf_remove(hx_api_object):
    ...
    if mf_job:
        ...
        for f in mf_job["files"]:
            ...
            if not f["downloaded"]:
                hxtool_global.hxtool_db.multiFileUpdateFile(
                    session["ht_profileid"], mf_job.doc_id, f["acquisition_id"]
                )
            ...
            if not ret and response_code != 404:
                app.logger.error(
                    "Failed to remove file acquisition {0} from the HX controller, response code: {1}".format(
                        f["acquisition_id"], response_code
                    )
                )
                success = False
        if success:
...
@ht_api.route(
    "/api/v{0}/acquisition/multi/mf/new".format(HXTOOL_API_VERSION), methods=["POST"]
)
@valid_session_required
def hxtool_api_acquisition_multi_mf_new(hx_api_object):
    ...
    if request.method == "POST":
        ...
        display_name = (
                ("display_name" in request.form)
                and request.form["display_name"]
                or "{0} job at {1}".format(session["ht_user"], datetime.datetime.now())
        )
        use_api_mode = "use_raw_mode" not in request.form
        ...
        if choices:
            ...
            for fl_id, file_ids in list(choices.items()):
                ...
                if not file_listing:
                    app.logger.warn(
                        "File Listing %s does not exist - User: %s@%s:%s",
                        session["ht_user"],
                        fl_id,
                        hx_api_object.hx_host,
                        hx_api_object.hx_port,
                    )
                    continue
                choice_files = [
                    file_listing["files"][i]
                    for i in file_ids
                    if i <= len(file_listing["files"])
                ]
                multi_file_eid = hxtool_global.hxtool_db.multiFileCreate(
                    session["ht_user"],
                    session["ht_profileid"],
                    display_name=display_name,
                    file_listing_id=file_listing.doc_id,
                    api_mode=use_api_mode,
                )
                ...
                for cf in choice_files:
                    if cf["hostname"] in agent_ids:
                        ...
                        (
                            ...
                            response_code,
                            response_data,
                        ) = hx_api_object.restListHosts(search_term=cf["hostname"])
                        agent_id = agent_ids[cf["hostname"]] = response_data["data"][
                            "entries"
                        ][0]["_id"]
                    path_split_char = "\\"
                    ...
                    (ret, response_code, response_data) = hx_api_object.restAcquireFile(
                        agent_id, path, filename, use_api_mode
                    )
                    if ret:
                        ...
                        mf_job_id = hxtool_global.hxtool_db.multiFileAddJob(
                            multi_file_eid, job_record
                        )
                        file_acquisition_task = hxtool_scheduler_task(
                            session["ht_profileid"],
                            "File Acquisition: {}".format(cf["hostname"]),
                        )
                        file_acquisition_task.add_step(
                            file_acquisition_task_module,
                            kwargs={
                        ...
                        app.logger.info(
                            format_activity_log(
                                msg="file acquistion requested",
                                fromhost=cf["hostname"],
                                path=cf["FullPath"],
                                host=agent_id,
                                user=session["ht_user"],
                                controller=session["hx_ip"],
            ...
            app.logger.error(
                format_activity_log(
                    error="Multi-file acquisition failed, no files were selected.",
                    action="new",
                    user=session["ht_user"],
                    controller=session["hx_ip"],
                )
...
@ht_api.route(
    "/api/v{0}/datatable_multi_filelisting".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def datatable_multi_filelisting(hx_api_object):
    ...
    for j in hxtool_global.hxtool_db.fileListingList(profile_id):
        ...
        # Completion rate
        bulk_download = hxtool_global.hxtool_db.bulkDownloadGet(
            bulk_download_eid=job["bulk_download_eid"]
        )
        if bulk_download:
            hosts_completed = len(
                [
                    _
                    for _ in bulk_download["hosts"]
                    if bulk_download["hosts"][_]["downloaded"]
                ]
            )
            job_progress = 0
            if hosts_completed > 0:
                job_progress = int(
                    hosts_completed / float(len(bulk_download["hosts"])) * 100
                )
            if "display_name" not in job:
                job["display_name"] = "hostset {0}, path: {1} regex: {2}".format(
                    bulk_download["hostset_id"], job["cfg"]["path"], job["cfg"]["regex"]
                )
        else:
            job_progress = job["file_count"] > 1 and 100 or 0
            if "display_name" not in job:
                job["display_name"] = "path: {0} regex: {1}".format(
                    job["cfg"]["path"], job["cfg"]["regex"]
                )

        job["progress"] = (
                "<div class='htMyBar htBarWrap'><div class='htBar' id='file_listing_prog_"
                + str(job["id"])
                + "' data-percent='"
                + str(job_progress)
                + "'></div></div>"
    ...
    return app.response_class(
        response=json.dumps({"data": data_rows}),
        status=200,
        mimetype="application/json",
    )
...
@ht_api.route(
    "/api/v{0}/datatable_multi_multifile".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def datatable_multi_multifile(hx_api_object):
    ...
    for mf in hxtool_global.hxtool_db.multiFileList(profile_id):
        ...
        job_progress = (
                ...
                and int(hosts_completed / float(job["file_count"]) * 100)
                or 0
        )
        job["progress"] = (
                "<div class='htMyBar htBarWrap'><div class='htBar' id='multi_file_prog_"
                + str(job["id"])
                + "' data-percent='"
                + str(job_progress)
                + "'></div></div>"
...
@ht_api.route(
    "/api/v{0}/datatable_stacking".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def datatable_stacking(hx_api_object):
        ...
        mydata["data"].append(
            {
                ...
                "bulk_acquisition_id": (
                    HXAPI.compat_str(bulk_download["bulk_acquisition_id"])
                    if "bulk_acquisition_id" in bulk_download
                    else "N/A"
                ),
    ...
    return app.response_class(
        response=json.dumps(mydata), status=200, mimetype="application/json"
    )
...
def datatable_ccc(hx_api_object):
    ...
    if ret:
        for channel in response_data["data"]["entries"]:
            myhostsets = [hostset["name"] for hostset in channel["host_sets"]]
            mydata["data"].append(
                {
...
@ht_api.route(
    "/api/v{0}/datatable/agentstatus/csv".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def datatable_agentstatus_csv(hx_api_object):
    ...
    (hret, hresponse_code, hresponse_data) = hx_api_object.restListHosts(
        limit=200000, filter_term={myField: myPattern}
    )
    ...
    return send_file(
        mem,
        attachment_filename="agent_statistics_" + myField + ".csv",
        as_attachment=True,
    )


@ht_api.route(
    "/api/v{0}/datatable/agentstatus".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def datatable_agentstatus(hx_api_object):
    ...
    (hret, hresponse_code, hresponse_data) = hx_api_object.restListHosts(
        limit=5000, filter_term={myField: myPattern}
    )
...
@ht_api.route(
    "/api/v{0}/datatable/avcontent".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def datatable_avcontent_detail(hx_api_object):
    ...
    for host in hresponse_data["data"]["entries"]:
        ...
        if "malware" in response_data["data"]:
            if (
                    "av" in response_data["data"]["malware"]
                    and myversion
                    == response_data["data"]["malware"]["av"]["content"]["version"]
            ):
                mydata["data"].append(
                    {
                        "hostname": host["hostname"],
...
@ht_api.route(
    "/api/v{0}/datatable/avengine".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
    ...
    for host in hresponse_data["data"]["entries"]:
        ...
        if "malware" in response_data["data"]:
            if (
                    "av" in response_data["data"]["malware"]
                    and myversion
                    == response_data["data"]["malware"]["av"]["engine"]["version"]
            ):
                mydata["data"].append(
                    {
                        "hostname": host["hostname"],
...
@ht_api.route(
    "/api/v{0}/datatable/avstatus".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
...
@ht_api.route(
    "/api/v{0}/datatable_categories".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
...
@ht_api.route(
    "/api/v{0}/datatable_indicators".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
...
@ht_api.route(
    "/api/v{0}/datatable_hosts_with_alerts".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def datatable_hosts_with_alerts(hx_api_object):
    if request.method == "GET":
        ...
        (ret, response_code, response_data) = hx_api_object.restListHosts(
            limit=request.args.get("limit"), sort_term="stats.alerts+descending"
        )
        if ret:
            for host in response_data["data"]["entries"]:
                myhosts.append(
                    [host["hostname"] + "___" + host["_id"], host["stats"]["alerts"]]
                )
        else:
            return ("", 500)
...
@ht_api.route(
    "/api/v{0}/datatable_alerts_host".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def datatable_alerts_host(hx_api_object):
    ...
    (ret, response_code, response_data) = hx_api_object.restGetAlerts(
        limit=request.args.get("limit"),
        filter_term={"agent._id": request.args.get("host")},
    )
    if ret:
        for alert in response_data["data"]["entries"]:
            ...
            if alert["source"] in hxtool_global.hx_alert_types:
                tname = js_path(
                    alert,
                    hxtool_global.hx_alert_types.get(alert["source"])["threat_key"],
                )
                if alert["source"] == "EXD":
    ...
    return app.response_class(
        response=json.dumps(myalerts), status=200, mimetype="application/json"
    )
...
def datatable_alerts(hx_api_object):
    ...
    if "source" in request.args:
        (ret, response_code, response_data) = hx_api_object.restGetAlerts(
            limit=request.args.get("limit"),
            filter_term={"source": request.args.get("source")},
        )
    else:
        (ret, response_code, response_data) = hx_api_object.restGetAlerts(
            limit=request.args.get("limit")
    ...
    if ret:
        for alert in response_data["data"]["entries"]:
            ...
            if hxtool_global.hxtool_config.get_child_item("apicache", "enabled", False):
                hresponse_data = hxtool_global.hxtool_db.cacheGet(
                    session["ht_profileid"], "host", alert["agent"]["_id"]
                )
            if hresponse_data == False:
                (
                    ...
                    hresponse_code,
                    hresponse_data,
                ) = hx_api_object.restGetHostSummary(alert["agent"]["_id"])
            ...
            myalerts["data"].append(
                [
                    HXAPI.compat_str(hostname)
                    + "___"
                    + HXAPI.compat_str(hid)
                    + "___"
                    + HXAPI.compat_str(aid),
                    domain,
                    alert["reported_at"],
                    alert["source"],
                    tname,
...
@ht_api.route(
    "/api/v{0}/datatable_alerts_full".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def datatable_alerts_full(hx_api_object):
    ...
    mylimit = int(request.args.get("limit")) if "limit" in request.args else None
    if "hostname" in request.args:
        (ret, response_code, response_data) = hx_api_object.restListHosts(
            search_term=request.args.get("hostname")
        )
        if ret:
            myhostlist = [
                hostname["_id"] for hostname in response_data["data"]["entries"]
            ]
            myfilters["agent._id"] = myhostlist

    if myfilters:
        (ret, response_code, response_data) = hx_api_object.restGetAlertsTime(
        ...
        (ret, response_code, response_data) = hx_api_object.restGetAlertsTime(
            request.args.get("startDate"), request.args.get("endDate"), limit=mylimit
        )
    if ret:
        ...
        if "alertname" in request.args:
            ...
            for alert in response_data:
                ...
                if (
                        alert["source"] == "EXD"
                        and myalertname in alert["event_values"]["process_name"]
                ):
                    myMatches.append(alert)
                if alert["source"] == "IOC":
                    if alert["condition"]["_id"] not in myiocs:
                        ...
                        if cret:
                            myiocs[alert["condition"]["_id"]] = cresponse_data["data"][
                                "entries"
                            ][0]
        ...
        for annotation in dbannotations:
            if annotation["hx_alert_id"] not in myannotations:
                myannotations[annotation["hx_alert_id"]] = {
                    "max_state": 0,
                    "count": len(annotation["annotations"]),
                }

            for item in annotation["annotations"]:
                if (
                        item["state"]
                        > myannotations[annotation["hx_alert_id"]]["max_state"]
                ):
                    myannotations[annotation["hx_alert_id"]]["max_state"] = item[
                        "state"
                    ]

        for alert in response_data:

            if alert["_id"] in myannotations:
            ...
            aid = alert["_id"]
            if HXAPI.compat_str(
                    myhosts[alert["agent"]["_id"]]["os"]["product_name"]
            ).startswith("Windows"):
                platform = "win"
            elif HXAPI.compat_str(
                    myhosts[alert["agent"]["_id"]]["os"]["product_name"]
            ).startswith("Mac"):
                platform = "mac"
            else:
            ...
            myalerts["data"].append(
                {
                    "DT_RowId": alert["_id"],
                    "platform": platform,
                    "hostname": HXAPI.compat_str(hostname)
                                + "___"
                                + HXAPI.compat_str(hid)
                                + "___"
                                + HXAPI.compat_str(aid),
...
def datatable_scripts(hx_api_object):
    if request.method == "GET":
        ...
        return app.response_class(
            response=json.dumps(myscripts), status=200, mimetype="application/json"
        )
...
def datatable_openioc(hx_api_object):
    if request.method == "GET":
        ...
        return app.response_class(
            response=json.dumps(myiocs), status=200, mimetype="application/json"
        )


@ht_api.route(
    "/api/v{0}/datatable_taskprofiles".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def datatable_taskprofiles(hx_api_object):
    if request.method == "GET":
        ...
        return app.response_class(
            response=json.dumps(mytaskprofiles), status=200, mimetype="application/json"
        )
...
def datatable_acqs(hx_api_object):
    if request.method != "GET":
        return
    (ret, response_code, response_data) = hx_api_object.restListAllAcquisitions(
        limit=500
    )
    if ret:
        myacqs = {"data": []}
        for acq in response_data["data"]["entries"]:
            if acq["type"] != "bulk":
                hresponse_data = False
                if hxtool_global.hxtool_config.get_child_item(
                        "apicache", "enabled", False
                ):
                    hresponse_data = hxtool_global.hxtool_db.cacheGet(
                        session["ht_profileid"], "host", acq["host"]["_id"]
                    )
                if hresponse_data == False:
                    (
                        hret,
                        hresponse_code,
                        hresponse_data,
                    ) = hx_api_object.restGetHostSummary(acq["host"]["_id"])
                if ret:
                    request_user = "N/A"
                    acq_url = None
                    if "url" in acq["acq"]:
                        ...
                        acq_url = "/hx/api/v3/acqs/{}/{}".format(
                            acq["type"], HXAPI.compat_str(acq["acq"]["_id"])
                        )
                    a_response_data = False
                    if hxtool_global.hxtool_config.get_child_item(
                            "apicache", "enabled", False
                    ):
                        a_response_data = hxtool_global.hxtool_db.cacheGet(
                            session["ht_profileid"], acq["type"], acq["acq"]["_id"]
                        )
                    if a_response_data == False:
                        (
                            ...
                            a_response_code,
                            a_response_data,
                        ) = hx_api_object.restGetUrl(acq_url)
                    else:
                        a_ret = True

                    if a_ret:
                        request_user = a_response_data["data"]["request_actor"][
                            "username"
                        ]
                    myacqs["data"].append(
                        {
                            ...
                            "hostname": hresponse_data["data"]["hostname"]
                                        + "___"
                                        + hresponse_data["data"]["_id"],
                            "domain": hresponse_data["data"]["domain"],
                            "containment_state": hresponse_data["data"][
                                "containment_state"
                            ],
                            "last_poll_timestamp": hresponse_data["data"][
                                "last_poll_timestamp"
                            ],
                            "platform": hresponse_data["data"]["os"]["platform"],
                            "product_name": hresponse_data["data"]["os"][
                                "product_name"
                            ],
                            "action": acq["acq"]["_id"],
                        }
        ...
        return app.response_class(
            response=json.dumps(myacqs), status=200, mimetype="application/json"
        )


@ht_api.route(
    "/api/v{0}/datatable_acqs_host".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def datatable_acqs_host(hx_api_object):
    if request.method != "GET":
        return
    (ret, response_code, response_data) = hx_api_object.restListAllAcquisitions(
        limit=500, filter_term={"host._id": request.args.get("host")}
    )
    if ret:
        myacqs = {"data": []}
        for acq in response_data["data"]["entries"]:
...
def datatable_es(hx_api_object):
    ...
    if ret:
            ...
            mysearches["data"].append(
                {
                    ...
                    "stat_searchstate_pending": search["stats"]["search_state"][
                        "PENDING"
                    ],
                    "stat_searchstate_matched": search["stats"]["search_state"][
                        "MATCHED"
                    ],
                    "stat_searchstate_notmatched": search["stats"]["search_state"][
                        "NOT_MATCHED"
                    ],
        ...
        return app.response_class(
            response=json.dumps(mysearches), status=200, mimetype="application/json"
        )
    else:
...
@ht_api.route(
    "/api/v{0}/datatable_indicatorqueue".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def datatable_indicatorqueue(hx_api_object):
    ...
    return app.response_class(
        response=json.dumps(myrules), status=200, mimetype="application/json"
    )
...
def datatable_bulk(hx_api_object):
    ...
    if ret:
        ...
        for acq in response_data["data"]["entries"]:
            ...
            if myhostsetid:
                if myhostsetid != 9:
                    (
                        ...
                        hresponse_code,
                        hresponse_data,
                    ) = hx_api_object.restListHostsets(filter_term={"_id": myhostsetid})
            ...
            # Comlete rate
            total_size = (
                    acq["stats"]["running_state"]["NEW"]
                    + acq["stats"]["running_state"]["QUEUED"]
                    + acq["stats"]["running_state"]["FAILED"]
                    + acq["stats"]["running_state"]["ABORTED"]
                    + acq["stats"]["running_state"]["DELETED"]
                    + acq["stats"]["running_state"]["REFRESH"]
                    + acq["stats"]["running_state"]["CANCELLED"]
                    + acq["stats"]["running_state"]["COMPLETE"]
            )
            if total_size == 0:
                completerate = 0
            else:
                completerate = int(
                    float(acq["stats"]["running_state"]["COMPLETE"])
                    / float(total_size)
                    * 100
                )

            completerate = min(completerate, 100)

            # Download rate
            bulk_download = hxtool_global.hxtool_db.bulkDownloadGet(
                profile_id=session["ht_profileid"], bulk_acquisition_id=acq["_id"]
            )
            ...
            if (
                    bulk_download
                    and bulk_download["task_profile"]
                    and bulk_download["task_profile"] in ["file_listing", "stacking"]
            ):
            ...
            mybulk["data"].append(
                {
                    ...
                    "running_state_cancelled": acq["stats"]["running_state"][
                        "CANCELLED"
                    ],
        ...
        return app.response_class(
            response=json.dumps(mybulk), status=200, mimetype="application/json"
        )
    else:
        return ("HX API Call failed", 500)
...
@ht_api.route(
    "/api/v{0}/datatable_es_result_types".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def datatable_es_result_types(hx_api_object):
    if not request.args.get("id"):
        return ("Missing search id", 404)
    (ret, response_code, response_data) = hx_api_object.restGetSearchResults(
        request.args.get("id"), limit=30000
    )
    if ret:
        mytypes = {}
        for host in response_data["data"]["entries"]:
            for event in host["results"]:
                if event["type"] not in mytypes:
                    mytypes[event["type"]] = ["hostname"]
                for key, val in event.items():
                    if key.replace(" ", "_") not in mytypes[event["type"]]:
                        if key == "data":
                            for datakey in val.keys():
                                if (
                                        datakey.replace(" ", "_")
                                        not in mytypes[event["type"]]
                                ):
                                    mytypes[event["type"]].append(
                                        datakey.replace(" ", "_")
                                    )
                        elif key == "type":
        ...
        return app.response_class(
            response=json.dumps(mytypes), status=200, mimetype="application/json"
        )
    else:
        return ("HX API Call failed", 500)
...
@ht_api.route(
    "/api/v{0}/datatable_es_result".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def datatable_es_result(hx_api_object):
    if not request.args.get("id") or not request.args.get("type"):
...
@ht_api.route(
    "/api/v{0}/chartjs_agentstatus".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def chartjs_agentstatus(hx_api_object):
    ...
    rData = {"labels": list(myData.keys()), "datasets": []}
    ...
    return app.response_class(
        response=json.dumps(rData), status=200, mimetype="application/json"
    )


@ht_api.route(
    "/api/v{0}/chartjs_malwarecontent".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def chartjs_malwarecontent(hx_api_object):
    if request.method != "GET":
        return
    myContent = {"none": 0}
    myData = {"labels": [], "datasets": []}
    (ret, response_code, response_data) = hx_api_object.restListHosts(limit=100000)
    if ret:
        for host in response_data["data"]["entries"]:
            (sret, sresponse_code, sresponse_data) = hx_api_object.restGetHostSysinfo(
                host["_id"]
            )
            if sret and "malware" in sresponse_data["data"].keys():
                if "av" in sresponse_data["data"]["malware"].keys():
                    if "content" in sresponse_data["data"]["malware"]["av"].keys():
                        if (
                                sresponse_data["data"]["malware"]["av"]["content"][
                                    "version"
                                ]
                                not in myContent
                        ):
    ...
    dataset = []
    mylist = [{"version": ckey, "count": cval} for ckey, cval in myContent.items()]
...
@ht_api.route(
    "/api/v{0}/chartjs_malwareengine".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
        ...
        for host in response_data["data"]["entries"]:
            ...
            if sret and "malware" in sresponse_data["data"].keys():
                if "av" in sresponse_data["data"]["malware"].keys():
                    if "content" in sresponse_data["data"]["malware"]["av"].keys():
                        if (
                                sresponse_data["data"]["malware"]["av"]["engine"]["version"]
                                not in myContent
                        ):
                            myContent[
                                sresponse_data["data"]["malware"]["av"]["engine"][
                                    "version"
                                ]
                            ] = 1
                        else:
...
@ht_api.route(
    "/api/v{0}/chartjs_malwarestatus".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def chartjs_malwarestatus(hx_api_object):
    ...
    if ret:
        for host in response_data["data"]["entries"]:
            ...
            if "MalwareProtectionStatus" in sresponse_data["data"].keys():
                if sresponse_data["data"]["MalwareProtectionStatus"] not in myContent:
    ...
    dataset = []
    mylist = [{"mode": ckey, "count": cval} for ckey, cval in myContent.items()]
    newlist = sorted(mylist, key=lambda k: k["count"])
    results = newlist[-10:]
...
@ht_api.route(
    "/api/v{0}/enterprise_search/chartjs_searches".format(HXTOOL_API_VERSION),
    methods=["GET"],
)
@valid_session_required
def hxtool_api_enterprise_search_chartjs_searches(hx_api_object):
    ...
    if ret:
        ...
        date_list = [
            endDate - datetime.timedelta(days=x) for x in range(delta.days + 1)
        ]
        mysearches = {date.strftime("%Y-%m-%d"): 0 for date in date_list[::-1]}
        for search in response_data["data"]["entries"]:

            if search["create_time"][0:10] in mysearches:
        ...
        return app.response_class(
            response=json.dumps(myr), status=rcode, mimetype="application/json"
        )
    else:
        return ("HX API Call failed", 500)
...
@ht_api.route(
    "/api/v{0}/acquisition/bulk/chartjs_acquisitions".format(HXTOOL_API_VERSION),
    methods=["GET"],
)
@valid_session_required
...
@ht_api.route(
    "/api/v{0}/chartjs_hosts_initial_agent_checkin".format(HXTOOL_API_VERSION),
    methods=["GET"],
)
@valid_session_required
def chartjs_hosts_initial_agent_checkin(hx_api_object):
    myhosts = {"labels": [], "datasets": []}
    ...
    # Generate data for all dates
    date_list = [endDate - datetime.timedelta(days=x) for x in range(delta.days + 1)]
    mycount = {date.strftime("%Y-%m-%d"): 0 for date in date_list[::-1]}

    (ret, response_code, response_data) = hx_api_object.restListHosts(limit=100000)
    if ret:
        for host in response_data["data"]["entries"]:
            if host["initial_agent_checkin"][0:10] in mycount:
    ...
    return app.response_class(
        response=json.dumps(myhosts), status=200, mimetype="application/json"
    )


@ht_api.route(
    "/api/v{0}/chartjs_events_timeline".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def chartjs_events_timeline(hx_api_object):
    ...
    date_list = [endDate - datetime.timedelta(days=x) for x in range(delta.days + 1)]
    mycount = {
        date.strftime("%Y-%m-%d"): {k: 0 for k in hxtool_global.hx_alert_types.keys()}
    ...
    if ret:
        for alert in response_data:
            ...
            if alert["event_at"][0:10] not in mycount:
                mycount[alert["event_at"][0:10]] = {
        ...
        return app.response_class(
            response=json.dumps(mydates), status=200, mimetype="application/json"
        )
    else:
        return ("", 500)
...
@ht_api.route(
    "/api/v{0}/chartjs_host_alert_timeline".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def chartjs_host_alert_timeline(hx_api_object):
    ...
    (ret, response_code, response_data) = hx_api_object.restGetAlerts(
        filter_term={"agent._id": request.args.get("id")}
    )
    if ret:
        for alert in response_data["data"]["entries"]:
            stats_dict[alert["source"]]["data"].append(
                {"x": alert["event_at"][0:19].replace("T", " "), "y": 1}
            )
...
@ht_api.route(
    "/api/v{0}/chartjs_events_distribution".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def chartjs_events_distribution(hx_api_object):
    ...
    mycount = {k: 0 for k in hxtool_global.hx_alert_types.keys()}
    ...
    if ret:
        for alert in response_data:
            ...
            if alert["source"] not in mycount:
...
@ht_api.route(
    "/api/v{0}/chartjs_inactive_hosts_per_hostset".format(HXTOOL_API_VERSION),
    methods=["GET"],
)
@valid_session_required
def chartjs_inactive_hosts_per_hostset(hx_api_object):
    ...
    if ret:
        ...
        for hostset in response_data["data"]["entries"]:
            (hret, hresponse_code, hresponse_data) = hx_api_object.restListHosts(
                query_terms={"host_sets._id": hostset["_id"]}
            )
            if hret:
                ...
                for host in hresponse_data["data"]["entries"]:
                    ...
                    if (int((now - x).total_seconds())) > int(
...
def profile():
    if request.method == "GET":
        ...
        if validate_json(["hx_name", "hx_host", "hx_port"], request_json):
            if hxtool_global.hxtool_db.profileCreate(
                    request_json["hx_name"],
                    request_json["hx_host"],
                    request_json["hx_port"],
            ):
...
@ht_api.route(
    "/api/v{0}/profile/<profile_id>".format(HXTOOL_API_VERSION),
...
def profile_by_id(profile_id):
    if request.method == "DELETE":
        ...
        request_json = request.json
        if validate_json(
                ["profile_id", "hx_name", "hx_host", "hx_port"], request_json
        ) and hxtool_global.hxtool_db.profileUpdate(
            request_json["_id"],
            request_json["hx_name"],
            request_json["hx_host"],
            request_json["hx_port"],
...
@ht_api.route(
    "/api/v{0}/stacking/<stack_job_eid>/results".format(HXTOOL_API_VERSION),
    methods=["GET"],
)
@valid_session_required
...
def cache_statistics(hx_api_object):
    ...
    if mystats["enabled"]:
        ...
        for k, v in hxtool_global.apicache["data"].items():
            mystats[k] = {
                "settings": v["settings"],
                "records processed": v["stats"]["records"],
            }
        return app.response_class(
            response=json.dumps(mystats), status=200, mimetype="application/json"
        )
    else:
        return app.response_class(
            response=json.dumps("DISABLED"), status=200, mimetype="application/json"
        )
...
@ht_api.route(
    "/api/v{0}/analysis/auditmodules".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
...
@ht_api.route(
    "/api/v{0}/analysis/auditdata".format(HXTOOL_API_VERSION), methods=["GET"]
)
@valid_session_required
def x15_analysis_auditdata(hx_api_object):
    generatorMeta = {"eventbuffer": "eventitem"}
...
def create_api_response(ret=True, response_code=200, response_data=False):
    api_response = {"api_success": ret, "api_response_code": response_code}
    if response_data:
        api_response["api_response"] = json.dumps(response_data)

    rcode = 200
    if not ret and response_code:
        rcode = response_code if response_code in [401, 404] else 400
