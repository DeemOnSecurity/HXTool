...
def formatHostsetsFabric(hs):
    ...
    for entry in hs["data"]["entries"]:
        ...
        x += "</span>"
        x += (
                '<span class="fe-dropdown__item-link-text" data-id="'
                + HXAPI.compat_str(entry["_id"])
                + '">'
                + entry["name"]
                + "</span>"
        )
        x += '<span class="fe-dropdown__item-link-right-section">'
        x += (
                '<span style="color: black;" class="fe-badge count-only">'
                + HXAPI.compat_str(entry["_id"])
                + "</span>"
        )
        x += "</span></a></li>"
