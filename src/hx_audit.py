...
def get_mime_type(generator):
    return (
            (generator in ["w32apifile-acquisition", "w32disk-acquisition"])
...
def get_audit_records(
        audit_data, generator, item_name, fields=None, post_process=None, **static_values
):
    items = []
...
class EmptyAuditException(Exception):
    pass
...
class AuditPackage:
    def __init__(self, acquisition_package_path):
        ...
        self.manifest = (
                ("manifest.json" in self.package.namelist())
                and json.loads(self.package.read("manifest.json").decode("utf-8"))
                or {}
        )
        self.audits = ("audits" in self.manifest) and self.manifest["audits"] or []
        self.metadata = (
                ("metadata.json" in self.package.namelist())
                and json.loads(self.package.read("metadata.json").decode("utf-8"))
                or None
        )
        ...
        if self.metadata:
            ...
            if sysinfo_audit:
                ...
                if sysinfo_audit["type"] == "application/xml":
                    self.hostname = (
                        ET.fromstring(
                            sysinfo_result, parser=ET.XMLParser(encoding="utf-8")
                        )
                            .find(".//hostname")
                            .text
                    )
                elif (
                        sysinfo_audit["type"] == "application/json"
                        and json.loads(sysinfo_result).get("SystemInfoItem", None)
                        is not None
                ):
                    self.hostname = json.loads(sysinfo_result)["SystemInfoItem"][0][
                        "hostname"
                    ]
    ...
    def get_audit(
            self, payload_name=None, generator=None, destination_path=None, open_only=False
    ):
                    ...
                    if result["type"] == "application/xml":
                        ...
                        if elem.tag == "itemList" and event == "start":
                            ...
                            if payload_item_tag is None:
                                raise EmptyAuditException(
                                    "The audit payload '{}' for generator '{}' is empty. Please manually inspect the audit package: {}".format(
                                        result["payload"],
                                        audit["generator"],
                        ...
                        if batch_mode:
                            result_dict = {
                                "results": [
                                    {audit_item: _} for _ in audit_json[audit_item]
                                ]
                            }
                            result_dict.update(d)
                            yield result_dict
