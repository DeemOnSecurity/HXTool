...
from src.hxtool_util import *
from src.hxtool_vars import default_encoding
...
class hxtool_scheduler:
    def __init__(self, thread_count=None):
        ...
        self._poll_thread = threading.Thread(
            target=self._scan_task_queue, name="PollThread"
        )
    ...
    def _add_task_api_task(self, profile_id, hx_host, hx_port, username, password):
        ...
        api_login_task.add_step(
            hxtool_task_modules.task_api_session_module,
            kwargs={
        ...
        for profile in profiles:
            ...
            if task_api_credential:
                try:
                    ...
                    logger.error(
                        "Please reset the background credential for {} ({}).".format(
                            profile["hx_host"], profile["profile_id"]
                        )
                    )
            else:
                logger.info(
                    "No background credential for {} ({}).".format(
                        profile["hx_host"], profile["profile_id"]
                    )
                )
    ...
    def add_task_api_session(self, profile_id, hx_host, hx_port, username, password):
        ...
        hxtool_global.hxtool_db.backgroundProcessorCredentialCreate(
            profile_id, username, HXAPI.b64(iv), HXAPI.b64(salt), encrypted_password
        )
    ...
    def signal_child_tasks(
            self, parent_task_id, parent_task_state, parent_stored_result
    ):
        with self._lock:
            ...
            with self._lock:
                if delete_children:
                    ...
                    for child_task_id in [
                        _.task_id
                        for _ in self.task_queue.values()
                    ...
                    for child_task_id in [
                        _["task_id"]
                        for _ in self.history_queue.values()
                        if _["parent_id"] == task_id
                    ]:
    ...
    def load_from_database(self):
        try:
            ...
            logger.error(
                "Failed to load saved tasks from the database. Error: {}".format(
                    pretty_exceptions(e)
                )
            )
...
class hxtool_scheduler_task:
    ...
    def _calculate_next_run(self):
        ...
        if self.state == TASK_STATE_PENDING_DELETION or (
                self.state == TASK_STATE_FAILED and self.stop_on_fail
        ):
            return
        elif self.state == TASK_STATE_QUEUED or self.state == TASK_STATE_RUNNING:
            logger.critical(
                "Task ID {} calculating next run while still running, this should never happen!".format(
                    self.task_id
                )
            )
        ...
        if self._defer_signal:
            ...
            self.next_run = self.last_run + datetime.timedelta(
                seconds=(self.defer_interval + random.randint(1, 15))
            )
        ...
        elif self.schedule and (
                (not self.end_time) or (self.end_time and (self.end_time < now))
        ):
            self.next_run = self.last_run + datetime.timedelta(
    ...
    def should_run(self):
        return (
                ...
                and self.enabled
                and self.state == TASK_STATE_SCHEDULED
                and (
                    self.parent_complete
                    if (self.parent_id and self.wait_for_parent)
                    else True
                )
                and datetime.datetime.utcnow() >= self.next_run
        )
        ...
        if self.enabled:
            ...
            with self._lock:
                ...
                for module, func, args, kwargs in self.steps:
                    logger.debug(
                        "Have module: {}, function: {}".format(module.__module__, func)
                    )
                    if getattr(module, "hxtool_task_module", lambda: False)():
                        if module.enabled == False:
                            logger.error(
                                "Module {} is disabled!".format(module.__module__)
                            )
                            ret = False
                        ...
                        for arg_i in module.input_args():
                            if not kwargs.get(arg_i["name"], None):
                                if arg_i["name"] in self.stored_result.keys():
                                    ...
                                    logger.error(
                                        "Module {} requires argument {} that was not found! Bailing!".format(
                                            module.__module__, arg_i["name"]
                                        )
                                    )
                                    ret = False
                                    self.state = TASK_STATE_FAILED
                    ...
                    if self.state != TASK_STATE_FAILED:
                        logger.debug(
                            "Begin execute {}.{}".format(module.__module__, func)
                        )
                        result = getattr(module, func)(*args, **kwargs)
                        logger.debug(
                            "End execute {}.{}".format(module.__module__, func)
                        )
                        if isinstance(result, tuple) and len(result) > 1:
                            ...
                            if isinstance(result[1], dict):
                                ...
                                logger.error(
                                    "Task module {} returned a value that was not a dictionary or None. Discarding the result.".format(
                                        module.__module__
                                    )
                                )
                        else:
                ...
                if not self.parent_id:
                    hxtool_global.hxtool_scheduler.signal_child_tasks(
                        self.task_id, self.state, self.stored_result
                    )
    ...
    def parent_state_callback(self, parent_task_id, parent_state, parent_stored_result):
        if self.parent_id == parent_task_id:
            logger.debug(
                "parent_state_callback(): task_id = {}, parent_id = {}, parent_state = {}".format(
                    self.task_id, parent_task_id, parent_state
                )
            )
    ...
    def store(self):
        if not (self.immutable or self._stored):
            ...
            hxtool_global.hxtool_db.taskUpdate(
                self.profile_id, self.task_id, self.serialize()
            )
    ...
    def serialize(self, include_module_data=True):
        ...
        if include_module_data:
            ...
            r["steps"] = [
                {"module": m.__module__, "function": f, "args": a, "kwargs": ka}
