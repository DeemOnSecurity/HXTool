...
class hxtool_session(CallbackDict, SessionMixin):
    ...
    def create(self):
        self.id = str(
            hmac.new(self.secret, os.urandom(32), digestmod=hashlib.sha256).hexdigest()
        )
...
class hxtool_session_interface(SessionInterface):
    def __init__(self, app, expiration_delta=30):
        ...
        session_reaper_task = hxtool_scheduler_task(
            "System", "Session Reaper", immutable=True
        )
        ...
        if session_id:
            ...
            if not cached_session:
                ...
                if session_record is not None:
                    ...
                    logger.debug(
                        "We have an existing database session with id: {0}".format(
                            session.id
                        )
                    )
            else:
                ...
                logger.debug(
                    "We have an existing cached session with id: {0}".format(session.id)
                )
    ...
    def session_reaper(self, app):
        ...
        for s in hxtool_global.hxtool_db.sessionList():
            if not s["update_timestamp"] or (
                    datetime.datetime.utcnow() - HXAPI.dt_from_str(s["update_timestamp"])
            ) >= (
                    app.permanent_session_lifetime
                    or datetime.timedelta(minutes=self.expiration_delta)
            ):
                logger.debug(
                    "Deleting session id: {} with update_timestamp: {}".format(
                        s["session_id"], s["update_timestamp"]
                    )
                )
