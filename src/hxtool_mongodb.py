...
try:
    ...
    print(
        "HXTool is configured to use MongoDB. Please install the 'pymongo' Python module"
    )
    exit(1)
...
import shlex
import re
from src import hxtool_logging
...
class hxtool_mongodb(hxtool_db):
    def __init__(
        ...
        try:
            self._client = MongoClient(
                ...
                document_class=tinydb_emulated_dict,
            )
            self._db_profile = self._client[db_name].profile
            self._db_background_processor_credential = self._client[
                db_name
            ].background_processor_credential
    ...
    def auditQuery(self, query, sort=None, qlimit=1000):
        ...
        if sort:
            return (
                self._db_audits.find(query)
                    .sort(sort["sortkey"], sort["operator"])
                    .limit(qlimit)
            )
        else:
        ...
        pipeline = [
            {
                "$group": {
                    "count": {"$sum": 1},
                    "_id": {"bulk_acquisition_id": "$bulk_acquisition_id"},
                }
            },
            {"$limit": 1000},
        ]
    ...
    def profileUpdate(self, profile_id, hx_name, hx_host, hx_port):
        return self._db_profile.replace_one(
            {"profile_id": profile_id},
    ...
    def backgroundProcessorCredentialCreate(
        ...
        return self._db_background_processor_credential.insert_one(
            {
                "profile_id": profile_id,
    ...
    def backgroundProcessorCredentialRemove(self, profile_id):
        return self._db_background_processor_credential.remove(
            {"profile_id": profile_id}
        )

    def backgroundProcessorCredentialGet(self, profile_id):
        return self._db_background_processor_credential.find_one(
            {"profile_id": profile_id}
        )
    ...
    def alertList(self, profile_id):
        return self.mongoStripKeys(
            list(self._db_alerts.find({"profile_id": profile_id}))
        )

    def alertGet(self, profile_id, hx_alert_id):
        return self.mongoStripKeys(
            self._db_alerts.find_one(
                {"profile_id": profile_id, "hx_alert_id": int(hx_alert_id)}
            )
        )
    ...
    def bulkDownloadGet(
        ...
        if bulk_download_eid:
            return self.mongoStripKeys(
                self._db_bulk_download.find_one({"_id": ObjectId(bulk_download_eid)})
            )
        elif profile_id and bulk_acquisition_id:
            return self.mongoStripKeys(
                self._db_bulk_download.find_one(
                    {
                        "profile_id": profile_id,
    ...
    def bulkDownloadList(self, profile_id):
        return self.mongoStripKeys(
            list(self._db_bulk_download.find({"profile_id": profile_id}))
        )
    ...
    def bulkDownloadUpdateHost(
        ...
        return self._db_bulk_download.update_one(
            {"_id": ObjectId(bulk_download_eid)}, {"$set": {"hosts." + host_id: d}}
        )

    def bulkDownloadDeleteHost(self, bulk_download_eid, host_id):
        return self._db_bulk_download.update_one(
            {"_id": ObjectId(bulk_download_eid)}, {"$unset": {"hosts." + host_id}}
        )
    ...
    def fileListingAddResult(self, profile_id, bulk_download_eid, result):
        return self._db_file_listing.update_one(
            {"profile_id": profile_id, "bulk_download_eid": str(bulk_download_eid)},
            {"$push": {"files": {"$each": result}}},
        )

    def fileListingGetByBulkId(self, profile_id, bulk_download_eid):
        return self._db_file_listing.find_one(
            {"profile_id": profile_id, "bulk_download_eid": str(bulk_download_eid)}
        )

    def fileListingGetById(self, flid):
        return self.mongoStripKeys(
            self._db_file_listing.find_one({"_id": ObjectId(flid)})
        )

    def fileListingList(self, profile_id):
        return self.mongoStripKeys(
            list(self._db_file_listing.find({"profile_id": profile_id}))
        )
    ...
    def multiFileAddJob(self, multi_file_id, job):
        return self._db_multi_file.update_one(
            {"_id": ObjectId(multi_file_id)}, {"$push": {"files": job}}
        )

    def multiFileList(self, profile_id):
        return self.mongoStripKeys(
            list(self._db_multi_file.find({"profile_id": profile_id}))
        )

    def multiFileGetById(self, multi_file_id):
        return self.mongoStripKeys(
            self._db_multi_file.find_one({"_id": ObjectId(multi_file_id)})
        )
    ...
    def multiFileUpdateFile(self, profile_id, multi_file_id, acquisition_id):
        return self._db_multi_file.update_one(
            ...
            {
                "$set": {
                    "files.$[].acquisition_id": acquisition_id,
    ...
    def stackJobGet(self, stack_job_eid=None, profile_id=None, bulk_download_eid=None):
        if stack_job_eid:
            ...
            return self._db_stacking.find_one(
                {
                    "profile_id": profile_id,
                    "bulk_download_eid": ObjectId(bulk_download_eid),
                }
            )
    ...
    def stackJobAddResult(self, profile_id, bulk_download_eid, hostname, result):
        ...
        return self._db_stacking.update_one(
            ...
            {"$set": {"hosts.$[].hostname": hostname, "hosts.$[].processed": True}},
        )
    ...
    def taskUpdate(self, profile_id, task_id, serialized_task):
        return self._db_tasks.replace_one(
            {"profile_id": profile_id, "task_id": task_id}, serialized_task
        )
    ...
    def taskProfileAdd(self, name, actor, params):
        return self._db_taskprofiles.insert_one(
            {
    ...
    def auditCreate(
        ...
        return self._db_audits.insert_one(
            {
                "profile_id": profile_id,
    ...
    def auditList(self, profile_id):
        return self.mongoStripKeys(
            list(self._db_audits.find({"profile_id": profile_id}))
        )

    def auditGet(self, profile_id, audit_id):
        return self.mongoStripKeys(
            self._db_audits.find_one({"profile_id": profile_id, "audit_id": audit_id})
        )
    ...
    def queryParse(self, myQuery):
        ...
        for part in mstr_parts:
            ...
            if myoperator == ":":
                ppart = part.split(":", 1)
                if len(ppart) > 1:
                    mysearch_dict["query"][ppart[0]] = {
                        "$regex": ".*" + re.escape(ppart[1]) + ".*"
                    }
            elif myoperator == "=":
                ppart = part.split("=", 1)
                if len(ppart) > 1:
            ...
            for part in myQueryParts:
                ...
                if part.startswith("groupby"):
                    ...
                    for myGroupPart in ppart:
                        ...
                        mygroup["_id"][myGroupPart.replace(".", "/")] = (
                                "$" + myGroupPart
                        )
