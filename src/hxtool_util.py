...
from src import hxtool_vars, hxtool_global, hxtool_logging
    ...
    def is_session_valid(*args, **kwargs):
        ...
        if session and "ht_user" in session and "ht_api_object" in session:
            ...
            if o.restIsSessionValid():
                ...
                logger.warn(
                    "The HX API token for the current session has expired, redirecting to the login page."
                )
        return ret
...
def validate_json(keys, j):
    return not any(k not in j or not j[k] for k in keys)
...
def crypt_aes(key, iv, data, decrypt=False, base64_coding=True):
    ...
    if decrypt:
        ...
        if 1 <= pad_length <= 15 and all(
                c == chr(pad_length) for c in data[-pad_length:]
        ):
...
def make_download_directory(hx_host, download_id, job_type=None):
    download_directory = combine_app_path(
        download_directory_base(), hx_host, str(download_id)
    )
    if job_type:
        download_directory = combine_app_path(
            download_directory_base(), hx_host, job_type, str(download_id)
        )
...
def submit_bulk_job(
        ...
        hostset_id=None,
        hosts={},
        hxtool_host_list_id=None,
        start_time=None,
        schedule=None,
        comment="HXTool Bulk Acquisition",
        download=True,
        task_profile=None,
        skip_base64=False,
):
    ...
    bulk_acquisition_task = hxtool_scheduler_task(
        session["ht_profileid"], "Bulk Acquisition ID: pending", start_time=start_time
    )
    if schedule:
    ...
    if download:
        bulk_download_eid = hxtool_global.hxtool_db.bulkDownloadCreate(
            session["ht_profileid"], hostset_id=hostset_id, task_profile=task_profile
        )
